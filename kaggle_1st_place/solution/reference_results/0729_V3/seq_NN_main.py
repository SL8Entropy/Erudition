...
def parse_args(argv=None):
    ...
    parser.add_argument("--finetune-epochs", type=int, help="Train a second unsimulated finetune phase for N epochs.")
...
def log_cfg_summary(cfg, train_wells, test_wells, include_test=True):
    ...
    log(
        ...
        f"window={getattr(cfg, 'pred_sg_smooth_window', 1025)}, "
        f"polyorder={getattr(cfg, 'pred_sg_smooth_polyorder', 1)}, "
        f"blend={getattr(cfg, 'pred_sg_smooth_blend', 0.6828376753426914):g}"
    )
