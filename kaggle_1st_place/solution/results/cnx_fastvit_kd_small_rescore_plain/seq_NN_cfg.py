...
UNET_MODE = "unet"
TRF_UNET_MODE = "trf_unet"
TWO_STAGE_UNET_MODE = "two_stage_unet"
BIDIR_CONVGRU_MODE = "bidir_convgru"
LEGACY_STAGE_UNET_MODE = "stage_unet"
MODEL_MODES = (UNET_MODE, TRF_UNET_MODE, TWO_STAGE_UNET_MODE, BIDIR_CONVGRU_MODE)
...
class CFG:
    ...
    # MD column-grid phase.  ``md_phase_shift`` moves the downsampling cut
    # lines back by that many raw rows when an item is built; ``md_phase_tta``
    # is the inference-time averaging over several such grids -- an int count
    # spread evenly over one column, an explicit sequence of row offsets, or
    # None/1 for the archived single-grid behaviour.
    md_phase_shift = 0
    md_phase_tta = None
    ...
    sim_cfg={
        ...
        'z_shift':{
            ...
            'noise_mode':{'TVT_bias_v2':1.0}, # 'copy'/'real_block'/'structured_random'/'TVT_bias'/'TVT_bias_v2'/'TVT_bias_v3'/'mixed_V1'/'conditional_GR'/'decomposed_GR'/'global_pool'
            ...
            'TVT_bias_v3_tvt_std_range':(0.14,0.34),
            'TVT_bias_v3_windows':(3072,768,96),
            'TVT_bias_v3_weights':(0.55,0.30,0.15),
            'TVT_bias_v3_step_prob':0.10,
            'TVT_bias_v3_step_abs_range':(3.0,7.0),
            'TVT_bias_v3_step_start_range':(0.25,0.80),
            'TVT_bias_v3_step_transition_rows':192,
            'TVT_bias_v3_max_abs_tvt':9.0,
            'TVT_bias_v3_response_power':0.9308,
            'TVT_bias_v3_response_weight':0.50,
            'TVT_bias_v3_mean_offset':0.03,
            'TVT_bias_v3_residual_std_range':(6.75,8.75),
            'TVT_bias_v3_residual_windows':(2048,1024,256,40,5,2),
            'TVT_bias_v3_residual_weights':(0.03,0.05,0.15,0.35,0.15,0.15,0.10),
            'TVT_bias_v3_residual_tail_df':6.0,
            'TVT_bias_v3_boundary_window':16,
            'TVT_bias_v3_boundary_decay_rows':32.0,
            'TVT_bias_v3_random_ramp_rows':2.0,
            'TVT_bias_v3_boundary_persist':0.0,
            'TVT_bias_v3_offset_std':1.20,
            'TVT_bias_v3_clip_abs':45.0,
        ...
        'reskin':{
            # Master-series re-skinning.  The trajectory shape is kept and the
            # well is re-placed into another typewell system, at the depth
            # quantile it occupied in the drilling band of its own system.
            'apply_prob':0.0,
            'noise_mode':'copy', # 'copy' keeps the current residual, 'real_block' pastes one
            'same_nan':True,
            'quantile_jitter':0.1,
            'max_attempts':8,
            'crop_margin_ft':None, # None -> cfg.typewell_window
            'grid_step_ft':1.0,
            'min_overlap_ft':100.0,
            'corr_threshold':0.9,
            'min_band_wells':3,
            'min_band_ft':20.0,
        },
    ...
    # Density sigmas are standard deviations, so equal values give the
    # Gaussian and Laplace targets equal variance.
    alignment_laplace_sigma = 1.25
    ...
    seed = 7
    epochs = 300
    # Number of initial training epochs for which a ConvNeXt U-Net backbone
    # stays frozen.  Non-ConvNeXt model modes ignore this setting.
    backbone_frozen_epochs = None
    log_freq = 5
    batch_size = 16
    val_batch_size = 16
    num_workers = 4
    ...
    seq_noise_power = 1.0
    # Emphasize suffix bins with locally distinctive horizontal-GR texture.
    gr_corr_weight = False
    gr_corr_weight_p = 1.0
    gr_corr_weight_smooth_window = 5
    gr_corr_weight_smooth_factor = 0.5
    min_epochs = 170
    ...
    finetune_disabled_augs = ["z_shift"]
    optimizer_cfg = {
        "optimizer": "adamw",
        "lr": 4e-4/2,
        "min_lr": 1e-6/2,
        ...
        "scheduler": "cos",
        "constant_ratio": 0.0,
        # Keep the historical single learning rate unless explicitly enabled.
        "layer_wise_lr": False,
        "layer_decay": 0.8,
    }
    ...
    # Model.
    model_name = "unet" # "unet"/"trf_unet"/"two_stage_unet"/"bidir_convgru"
    ...
    unet_cfg = {
        ...
        # ConvNeXt decoder-only auxiliary heads.  The final head keeps full
        # weight; decoder stages are weighted by successive powers of the
        # stage decay during training.
        "deep_sup": False,
        "deep_sup_stage_decay": 0.5,
    }
    trf_unet_cfg = {
        # Supported backbone_name values and exact pretrained timm checkpoints:
        #   "caformer_s18":       caformer_s18.sail_in22k_ft_in1k_384 (Apache-2.0)
        #   "swin_tiny":          swin_tiny_patch4_window7_224.ms_in22k_ft_in1k (MIT)
        #   "swin_small":         swin_small_patch4_window7_224.ms_in22k_ft_in1k (MIT)
        #   "swin_base":          swin_base_patch4_window7_224.ms_in22k_ft_in1k (MIT)
        #   "pvt_v2_b2":          pvt_v2_b2.in1k (Apache-2.0)
        #   "twins_svt_small":    twins_svt_small.in1k (Apache-2.0)
        #   "maxvit_rmlp_small":  maxvit_rmlp_small_rw_224.sw_in1k (Apache-2.0)
        #   "coatnet_1":          coatnet_rmlp_1_rw2_224.sw_in12k_ft_in1k (Apache-2.0)
        #   "coatnet_2":          coatnet_2_rw_224.sw_in12k_ft_in1k (Apache-2.0)
        # Hiera and DINOv3 are intentionally absent: their checkpoint licenses
        # are not commercially permissive without separate legal review.
        "backbone_name": "swin_small",
        # None loads pretrained weights for training/common construction and
        # skips the download while reconstructing already trained submit models.
        "pretrained": None,
        "emb_dim": 32,
        "pad_value": 0.0,
        "drop_path_rate": 0.0,
        # These normalization/activation settings belong only to the custom
        # ResBlock decoder.  The Transformer backbone keeps native norms.
        "res_kernel_size": 3,
        "resblock_act": "silu",
        "resblock_norm": "BN",
        "dropout": 0.05,
        ...
        "channel_mask_all_prob": 0.0,
        "channel_mask_value": 0.0,
        "deep_sup": False,
        "deep_sup_stage_decay": 0.5,
    }
    two_stage_unet_cfg = {
    ...
    bidir_convgru_cfg = {
        "bidir_convgru_hidden_dim": 32,
        "bidir_convgru_layers": 2,
        "bidir_convgru_kernel_size": 5,
        "bidir_convgru_dropout": 0.05,
        "bidir_convgru_norm": "LN",
        "bidir_convgru_act": "silu",
        "head_hidden_mult": 2,
        "regression_head_mode": "logits",
        "share_logits_head": True,
        "offset_detach_logits_head_in_loss": False,
    ...
    pred_sg_smooth = True
    pred_sg_smooth_window = 577
    pred_sg_smooth_polyorder = 1
    pred_sg_smooth_blend = 0.7261960515627183
    ...
    def set_model_name(self, model_name):
        ...
        model_cfg_attr = {
            UNET_MODE: "unet_cfg",
            TRF_UNET_MODE: "trf_unet_cfg",
            TWO_STAGE_UNET_MODE: "two_stage_unet_cfg",
            BIDIR_CONVGRU_MODE: "bidir_convgru_cfg",
        }[model_name]
        self.model_cfg = getattr(self, model_cfg_attr)
    ...
    def _migrate_legacy_names(self):
        ...
        if "gr_corr_weight" not in instance_attrs:
            self.gr_corr_weight = False
        if "gr_corr_weight_p" not in instance_attrs:
            self.gr_corr_weight_p = 1.0
        if "gr_corr_weight_smooth_window" not in instance_attrs:
            self.gr_corr_weight_smooth_window = 5
        if "gr_corr_weight_smooth_factor" not in instance_attrs:
            self.gr_corr_weight_smooth_factor = 0.5
        if "finetune_disabled_augs" not in instance_attrs:
            self.finetune_disabled_augs = deepcopy(CFG.finetune_disabled_augs)
        if "sim_cfg" in instance_attrs and "z_shift" in self.sim_cfg:
            self.sim_cfg["z_shift"].setdefault("safe_range_guard", False)
            self.sim_cfg["z_shift"].setdefault("safe_range_guard_resample", 5)
        if "sim_cfg" in instance_attrs:
            # Archived configs predate master-series re-skinning.
            if "reskin" not in self.sim_cfg:
                self.sim_cfg["reskin"] = deepcopy(CFG.sim_cfg["reskin"])
            else:
                for key, value in CFG.sim_cfg["reskin"].items():
                    self.sim_cfg["reskin"].setdefault(key, deepcopy(value))
        if "md_phase_shift" not in instance_attrs:
            self.md_phase_shift = CFG.md_phase_shift
        if "md_phase_tta" not in instance_attrs:
            self.md_phase_tta = CFG.md_phase_tta
        if "unet_cfg" not in instance_attrs and "stage_unet_cfg" in instance_attrs:
            self.unet_cfg = self.stage_unet_cfg
        if "trf_unet_cfg" not in instance_attrs:
            self.trf_unet_cfg = deepcopy(CFG.trf_unet_cfg)
        ...
        if "unet_loss_weights" in instance_attrs:
            # Archived configs predate the optional ordered-axis CDF term.
            self.unet_loss_weights.setdefault("CDF", 0.0)
        ...
        if "alignment_laplace_sigma" not in instance_attrs:
            self.alignment_laplace_sigma = CFG.alignment_laplace_sigma
        ...
        for model_cfg_name in ("unet_cfg", "two_stage_unet_cfg", "bidir_convgru_cfg"):
...
def resolve_finetune_disabled_augs(cfg):
    disabled_augs = getattr(cfg, "finetune_disabled_augs", CFG.finetune_disabled_augs)
    if isinstance(disabled_augs, (str, bytes)) or not isinstance(disabled_augs, (list, tuple)):
        raise TypeError("finetune_disabled_augs must be a list or tuple of augmentation names")
    if any(not isinstance(name, str) for name in disabled_augs):
        raise TypeError("finetune_disabled_augs entries must be strings")

    disabled_augs = tuple(disabled_augs)
    duplicated = sorted({name for name in disabled_augs if disabled_augs.count(name) > 1})
    if duplicated:
        raise ValueError(f"finetune_disabled_augs contains duplicate names: {duplicated}")

    available_augs = tuple(dict.fromkeys((*cfg.sim_cfg, *cfg.aug_cfg)))
    unknown = sorted(set(disabled_augs) - set(available_augs))
    if unknown:
        raise ValueError(
            f"finetune_disabled_augs contains unknown names: {unknown}; "
            f"available names: {list(available_augs)}"
        )
    return disabled_augs
...
def make_trf_unet_cfg(*, model_overrides=None, loss_weights=None, **overrides):
    cfg = CFG(model_name=TRF_UNET_MODE, seq_target_mode=UNET_MODE, **overrides)
    if model_overrides:
        cfg.trf_unet_cfg.update(model_overrides)
        cfg.set_model_name(TRF_UNET_MODE)
    if loss_weights is not None:
        cfg.unet_loss_weights = {
            key: float(loss_weights.get(key, 0.0))
            for key in cfg.unet_loss_weights
...
def make_bidir_convgru_cfg(*, model_overrides=None, loss_weights=None, **overrides):
    cfg = CFG(model_name=BIDIR_CONVGRU_MODE, seq_target_mode=UNET_MODE, **overrides)
    if model_overrides:
        cfg.bidir_convgru_cfg.update(model_overrides)
    if loss_weights is not None:
        cfg.unet_loss_weights = {
            key: float(loss_weights.get(key, 0.0))
            for key in cfg.unet_loss_weights
...
SEQ_RANDOM_SEARCH_SPACE = {
    ...
    "z_shift_noise_mode": (
        ...
        {"TVT_bias_v2": 0.5, "TVT_bias_v3": 0.5},
    ),
    ...
    for name, cfg in cfgs:
        ...
        if model_cfg is None:
            if model_name == TWO_STAGE_UNET_MODE:
                model_cfg = cfg.two_stage_unet_cfg
            elif model_name == TRF_UNET_MODE:
                model_cfg = cfg.trf_unet_cfg
            elif model_name == BIDIR_CONVGRU_MODE:
                model_cfg = cfg.bidir_convgru_cfg
            else:
                model_cfg = cfg.unet_cfg
        if model_name == TRF_UNET_MODE:
            trf_model_cfg = model_cfg
            backbone_name = str(trf_model_cfg.get("backbone_name", ""))
            if backbone_name not in TRF_UNET_BACKBONES:
                raise ValueError(
                    f"{name}: unknown trf_unet backbone_name={backbone_name!r}; "
                    f"expected one of {sorted(TRF_UNET_BACKBONES)}"
                )
            pretrained = trf_model_cfg.get("pretrained", None)
            if pretrained is not None and not isinstance(pretrained, bool):
                raise ValueError(f"{name}: trf_unet pretrained must be boolean or None")
            if any(key.startswith("unet_convnext_") for key in trf_model_cfg):
                raise ValueError(
                    f"{name}: trf_unet keeps native backbone normalization and does not "
                    "accept ConvNeXt normalization-replacement settings"
                )
            for field_name in ("emb_dim", "res_kernel_size"):
                if int(trf_model_cfg.get(field_name, 0)) <= 0:
                    raise ValueError(f"{name}: trf_unet {field_name} must be positive")
            if int(trf_model_cfg.get("res_kernel_size", 0)) % 2 == 0:
                raise ValueError(f"{name}: trf_unet res_kernel_size must be odd")
            pad_value = float(trf_model_cfg.get("pad_value", 0.0))
            if not math.isfinite(pad_value):
                raise ValueError(f"{name}: trf_unet pad_value must be finite")
            channel_mask_value = float(trf_model_cfg.get("channel_mask_value", 0.0))
            if not math.isfinite(channel_mask_value):
                raise ValueError(f"{name}: trf_unet channel_mask_value must be finite")
            for field_name in (
                "dropout",
                "drop_path_rate",
                "channel_mask_prob",
                "channel_mask_all_prob",
            ):
                field_value = float(trf_model_cfg.get(field_name, 0.0))
                if not math.isfinite(field_value) or not 0.0 <= field_value <= 1.0:
                    raise ValueError(
                        f"{name}: trf_unet {field_name} must be finite and in [0, 1]"
                    )
            trf_deep_sup = trf_model_cfg.get("deep_sup", False)
            if not isinstance(trf_deep_sup, bool):
                raise ValueError(f"{name}: trf_unet deep_sup must be boolean")
            trf_deep_sup_decay = float(trf_model_cfg.get("deep_sup_stage_decay", 0.5))
            if not math.isfinite(trf_deep_sup_decay) or not 0.0 <= trf_deep_sup_decay <= 1.0:
                raise ValueError(
                    f"{name}: trf_unet deep_sup_stage_decay must be finite and in [0, 1]"
                )
            model_cfg = {
                "unet_arch": "unet",
                "unet_mults": (1,),
                "unet_stacks": 1,
                "unet_stem_stride": (1, 1),
                "unet_stem_down_mode": "pool",
                "unet_convnext_norm_replace": "LN",
                "unet_dropout": trf_model_cfg.get("dropout", 0.0),
                "unet_channel_mask_prob": trf_model_cfg.get("channel_mask_prob", 0.0),
                "unet_channel_mask_all_prob": trf_model_cfg.get("channel_mask_all_prob", 0.0),
                "deep_sup": False,
                **trf_model_cfg,
            }
        elif model_name == BIDIR_CONVGRU_MODE:
            # Keep the model dictionary independent while supplying only the
            # compatibility defaults needed by the shared U-Net validator.
            model_cfg = {
                "unet_arch": "unet",
                "unet_mults": (1,),
                "unet_stacks": 1,
                ...
                "unet_dropout": model_cfg.get("bidir_convgru_dropout", 0.0),
                "unet_channel_mask_prob": model_cfg.get("unet_channel_mask_prob", 0.0),
                "unet_channel_mask_all_prob": model_cfg.get("unet_channel_mask_all_prob", 0.0),
                **model_cfg,
            }
        ...
        if model_name == BIDIR_CONVGRU_MODE:
            hidden_dim = int(model_cfg.get("bidir_convgru_hidden_dim", 32))
            layers = int(model_cfg.get("bidir_convgru_layers", 2))
            kernel_size = int(model_cfg.get("bidir_convgru_kernel_size", 5))
            dropout = float(model_cfg.get("bidir_convgru_dropout", 0.0))
            norm = str(model_cfg.get("bidir_convgru_norm", "LN"))
            act = str(model_cfg.get("bidir_convgru_act", "silu"))
            if hidden_dim <= 0 or hidden_dim % 2 != 0:
                raise ValueError(f"{name}: bidir_convgru_hidden_dim must be a positive even integer")
            if layers <= 0:
                raise ValueError(f"{name}: bidir_convgru_layers must be positive")
            if kernel_size <= 0 or kernel_size % 2 == 0:
                raise ValueError(f"{name}: bidir_convgru_kernel_size must be a positive odd integer")
            if not 0.0 <= dropout <= 1.0:
                raise ValueError(f"{name}: bidir_convgru_dropout must be in [0, 1]")
            if norm not in {"BN", "IN", "LN"}:
                raise ValueError(f"{name}: unknown bidir_convgru_norm={norm!r}")
            if act not in {"silu", "relu"}:
                raise ValueError(f"{name}: unknown bidir_convgru_act={act!r}")
        ...
        safe_range_guard = z_shift_cfg.get("safe_range_guard", False)
        if not isinstance(safe_range_guard, bool):
            raise TypeError(
                f"{name}: safe_range_guard must be a boolean, "
                f"got {type(safe_range_guard).__name__}"
            )
        safe_range_guard_resample = z_shift_cfg.get("safe_range_guard_resample", 5)
        if isinstance(safe_range_guard_resample, bool) or not isinstance(safe_range_guard_resample, int):
            raise TypeError(
                f"{name}: safe_range_guard_resample must be a positive integer, "
                f"got {safe_range_guard_resample!r}"
            )
        if safe_range_guard_resample <= 0:
            raise ValueError(
                f"{name}: safe_range_guard_resample must be a positive integer, "
                f"got {safe_range_guard_resample}"
            )
        prefix_sample_len = z_shift_cfg.get("prefix_sample_len")
        if prefix_sample_len is not None and int(prefix_sample_len) < 0:
            raise ValueError(f"{name}: prefix_sample_len must be non-negative or None, got {prefix_sample_len}")
        ...
        v3_tvt_range = tuple(float(v) for v in z_shift_cfg.get("TVT_bias_v3_tvt_std_range", ()))
        if len(v3_tvt_range) != 2 or any(not math.isfinite(v) or v < 0.0 for v in v3_tvt_range) or v3_tvt_range[0] > v3_tvt_range[1]:
            raise ValueError(f"{name}: TVT_bias_v3_tvt_std_range must be a finite non-negative ordered pair")
        v3_windows = tuple(int(v) for v in z_shift_cfg.get("TVT_bias_v3_windows", ()))
        if len(v3_windows) != 3 or any(v <= 0 for v in v3_windows):
            raise ValueError(f"{name}: TVT_bias_v3_windows must contain three positive integers, got {v3_windows!r}")
        v3_weights = tuple(float(v) for v in z_shift_cfg.get("TVT_bias_v3_weights", ()))
        if len(v3_weights) != 3 or any(not math.isfinite(v) or v < 0.0 for v in v3_weights) or sum(v3_weights) <= 0.0:
            raise ValueError(f"{name}: TVT_bias_v3_weights must be finite non-negative values with positive sum")
        v3_step_prob = float(z_shift_cfg.get("TVT_bias_v3_step_prob", 0.0))
        if not math.isfinite(v3_step_prob) or not 0.0 <= v3_step_prob <= 1.0:
            raise ValueError(f"{name}: TVT_bias_v3_step_prob must be finite and in [0, 1], got {v3_step_prob}")
        for key in ("TVT_bias_v3_step_abs_range", "TVT_bias_v3_step_start_range", "TVT_bias_v3_residual_std_range"):
            value_range = tuple(float(v) for v in z_shift_cfg.get(key, ()))
            if len(value_range) != 2 or any(not math.isfinite(v) for v in value_range) or value_range[0] < 0.0 or value_range[0] > value_range[1]:
                raise ValueError(f"{name}: {key} must be a finite ordered pair with a non-negative lower bound")
        step_start_range = tuple(float(v) for v in z_shift_cfg.get("TVT_bias_v3_step_start_range", ()))
        if step_start_range[1] > 1.0:
            raise ValueError(f"{name}: TVT_bias_v3_step_start_range upper bound must be <= 1")
        v3_step_transition = int(z_shift_cfg.get("TVT_bias_v3_step_transition_rows", 0))
        if v3_step_transition <= 0:
            raise ValueError(f"{name}: TVT_bias_v3_step_transition_rows must be positive")
        v3_response_power = float(z_shift_cfg.get("TVT_bias_v3_response_power", 0.0))
        if not math.isfinite(v3_response_power) or v3_response_power <= 0.0:
            raise ValueError(
                f"{name}: TVT_bias_v3_response_power must be finite and positive, got {v3_response_power}"
            )
        v3_response_weight = float(z_shift_cfg.get("TVT_bias_v3_response_weight", 0.0))
        if not math.isfinite(v3_response_weight) or v3_response_weight < 0.0:
            raise ValueError(
                f"{name}: TVT_bias_v3_response_weight must be finite and non-negative, got {v3_response_weight}"
            )
        v3_mean_offset = float(z_shift_cfg.get("TVT_bias_v3_mean_offset", 0.0))
        if not math.isfinite(v3_mean_offset):
            raise ValueError(f"{name}: TVT_bias_v3_mean_offset must be finite, got {v3_mean_offset}")
        for key in ("TVT_bias_v3_max_abs_tvt", "TVT_bias_v3_offset_std", "TVT_bias_v3_clip_abs"):
            value = float(z_shift_cfg.get(key, 0.0))
            if not math.isfinite(value) or value < 0.0:
                raise ValueError(f"{name}: {key} must be finite and non-negative, got {value}")
        v3_residual_windows = tuple(int(v) for v in z_shift_cfg.get("TVT_bias_v3_residual_windows", ()))
        if len(v3_residual_windows) == 0 or any(v <= 0 for v in v3_residual_windows):
            raise ValueError(f"{name}: TVT_bias_v3_residual_windows must contain positive integers")
        v3_residual_weights = tuple(float(v) for v in z_shift_cfg.get("TVT_bias_v3_residual_weights", ()))
        if len(v3_residual_weights) != len(v3_residual_windows) + 1 or any(not math.isfinite(v) or v < 0.0 for v in v3_residual_weights) or sum(v3_residual_weights) <= 0.0:
            raise ValueError(f"{name}: TVT_bias_v3_residual_weights must match windows and have positive non-negative weights")
        v3_tail_df = float(z_shift_cfg.get("TVT_bias_v3_residual_tail_df", 0.0))
        if not math.isfinite(v3_tail_df) or v3_tail_df <= 0.0:
            raise ValueError(f"{name}: TVT_bias_v3_residual_tail_df must be finite and positive")
        v3_boundary_window = int(z_shift_cfg.get("TVT_bias_v3_boundary_window", 0))
        if v3_boundary_window <= 0:
            raise ValueError(f"{name}: TVT_bias_v3_boundary_window must be positive")
        for key in ("TVT_bias_v3_boundary_decay_rows", "TVT_bias_v3_random_ramp_rows"):
            value = float(z_shift_cfg.get(key, 0.0))
            if not math.isfinite(value) or value < 0.0:
                raise ValueError(f"{name}: {key} must be finite and non-negative")
        v3_boundary_persist = float(z_shift_cfg.get("TVT_bias_v3_boundary_persist", 0.0))
        if not math.isfinite(v3_boundary_persist) or not 0.0 <= v3_boundary_persist <= 1.0:
            raise ValueError(f"{name}: TVT_bias_v3_boundary_persist must be finite and in [0, 1]")
        ...
        if not math.isfinite(drop_path_rate) or not 0.0 <= drop_path_rate <= 1.0:
            raise ValueError(
                f"{name}: drop_path_rate must be finite and in [0, 1], got {drop_path_rate}"
            )
        ...
        deep_sup = model_cfg.get("deep_sup", False)
        if not isinstance(deep_sup, bool):
            raise ValueError(f"{name}: deep_sup must be boolean, got {deep_sup!r}")
        deep_sup_stage_decay = float(model_cfg.get("deep_sup_stage_decay", 0.5))
        if (
            not math.isfinite(deep_sup_stage_decay)
            or not 0.0 <= deep_sup_stage_decay <= 1.0
        ):
            raise ValueError(
                f"{name}: deep_sup_stage_decay must be finite and in [0, 1], "
                f"got {deep_sup_stage_decay}"
            )
        if deep_sup and not (
            model_name == TRF_UNET_MODE
            or (model_name == UNET_MODE and unet_arch == "convnext_small")
        ):
            raise ValueError(
                f"{name}: deep_sup=True is supported only for model_name='trf_unet' "
                "or model_name='unet' with unet_arch='convnext_small'"
            )
        ...
        layer_wise_lr = optimizer_cfg.get("layer_wise_lr", False)
        if not isinstance(layer_wise_lr, bool):
            raise ValueError(
                f"{name}: optimizer_cfg layer_wise_lr must be boolean, got {layer_wise_lr!r}"
            )
        layer_decay = float(optimizer_cfg.get("layer_decay", 0.8))
        if layer_wise_lr and (not math.isfinite(layer_decay) or not 0.0 < layer_decay <= 1.0):
            raise ValueError(
                f"{name}: optimizer_cfg layer_decay must be finite and in (0, 1] when "
                f"layer_wise_lr is enabled, got {layer_decay}"
            )
        ...
        gr_corr_weight_p = float(getattr(cfg, "gr_corr_weight_p", 1.0))
        if not math.isfinite(gr_corr_weight_p) or gr_corr_weight_p <= 0.0:
            raise ValueError(
                f"{name}: gr_corr_weight_p must be finite and positive, "
                f"got {gr_corr_weight_p}"
            )
        gr_corr_weight_smooth_window = getattr(cfg, "gr_corr_weight_smooth_window", 5)
        try:
            parsed_gr_corr_weight_smooth_window = int(gr_corr_weight_smooth_window)
        except (TypeError, ValueError, OverflowError):
            parsed_gr_corr_weight_smooth_window = 0
        if isinstance(gr_corr_weight_smooth_window, bool) or (
            parsed_gr_corr_weight_smooth_window != gr_corr_weight_smooth_window
            or parsed_gr_corr_weight_smooth_window <= 0
        ):
            raise ValueError(
                f"{name}: gr_corr_weight_smooth_window must be a positive integer, "
                f"got {gr_corr_weight_smooth_window}"
            )
        gr_corr_weight_smooth_factor = float(
            getattr(cfg, "gr_corr_weight_smooth_factor", 0.5)
        )
        if not math.isfinite(gr_corr_weight_smooth_factor) or gr_corr_weight_smooth_factor < 0.0:
            raise ValueError(
                f"{name}: gr_corr_weight_smooth_factor must be finite and non-negative, "
                f"got {gr_corr_weight_smooth_factor}"
            )
        epochs = int(getattr(cfg, "epochs", 0))
        if epochs <= 0:
            raise ValueError(f"{name}: epochs must be positive, got {epochs}")
        backbone_frozen_epochs = getattr(cfg, "backbone_frozen_epochs", None)
        if backbone_frozen_epochs is not None and int(backbone_frozen_epochs) < 0:
            raise ValueError(
                f"{name}: backbone_frozen_epochs must be non-negative or None, "
                f"got {backbone_frozen_epochs!r}"
            )
        ...
        resolve_finetune_disabled_augs(cfg)
...
def make_seq_requested_ablation_cfgs():
    v1_cfg = make_aug_cfg(
        ...
        GR_trf={"a_std": 0.05},
        typewell_drift={"apply_prob": 0.3},
        model_overrides={"unet_dropout": 0.0},
        cv_repeats=3,
        cv_split_seed=42,
        early_stopping_rounds=50,
    )
    v1_cfg.sim_cfg["z_shift"].update(
        {
            "apply_prob": 0.85,
            "nan_mode": {"shift": 1.0},
        }
...
NEIGHBOR_WELL_CHANNELS = (
    # Every channel the pipeline derives from other wells' interpretations via
    # the XY geo prior.  ``geo_tvt_diff`` is already in the 0801_V2 recipe; the
    # rest are the "neighbouring-well information" that looked good in CV.
    "geo_s_rel",
    "geo_tvt_rel",
    "geo_tvt_abs_diff",
    "geo_dS",
...
def make_bilzard_cfgs():
    """Four forks of 0801_V2 that port ideas from the 2nd-place solution.

    ``rb_v1_neighbor``  all neighbouring-well channels on.  This is not a
                        proposed improvement, it is the candidate that the
                        leave-largest-contribution-out acceptance test exists
                        to judge (see ``seq_NN_robust_compare.py``).
    ``rb_v2_synth``     physically-grounded synthesis: trajectory mixup,
                        master-series re-skinning, real residual pasting.
    ``rb_v3_tta``       MD column-grid phase averaging at inference only.
    ``rb_v4_all``       v2 and v3 together.

    Every entry inherits the rest of 0801_V2 unchanged, so any difference in
    the holdout score is attributable to the listed knobs.
    """

    base = dict(make_seq_requested_ablation_cfgs())["submit_ver_0801_V2"]

    neighbor_cfg = deepcopy(base)
    neighbor_cfg.unet_static_channels = _with_features(
        neighbor_cfg.unet_static_channels,
        NEIGHBOR_WELL_CHANNELS,
    )
    neighbor_cfg.refresh()

    synth_cfg = deepcopy(base)
    synth_cfg.sim_cfg["z_shift"].update(
        {
            # Blend two real suffix paths instead of only bootstrapping one.
            "trend_mixup_prob": 0.5,
            "trend_mixup_lam_range": (0.25, 0.75),
            # Keep the fitted GR-residual model as the majority mode, but give
            # real pasted residuals a share of the samples.
            "noise_mode": {"TVT_bias_v2": 0.7, "real_block": 0.3},
        }
    )
    synth_cfg.sim_cfg["reskin"].update(
        {
            "apply_prob": 0.5,
            "noise_mode": "copy",
            "quantile_jitter": 0.1,
    ...
    tta_cfg = deepcopy(base)
    tta_cfg.md_phase_tta = 8
    tta_cfg.refresh()

    all_cfg = deepcopy(synth_cfg)
    all_cfg.md_phase_tta = 8
    all_cfg.refresh()

    # --- single-change forks added for the PF-channel and backbone-size questions -------
    # pf_v1: exactly the two PF channels the author added in their own 0803_V2 snapshot,
    # and nothing else (0803_V2 also added two geo channels, which confounded it).
    pf_cfg = deepcopy(base)
    pf_cfg.unet_static_channels = _with_features(
        pf_cfg.unet_static_channels, ("pf_particle_density_prob", "pf_prob_ffbsi"))
    pf_cfg.refresh()

    # Backbone size.  ``unet_arch`` is only a label the builder validates; the network is
    # whatever ``unet_timm_model_name`` names, and stage widths are read from the model.
    # tiny keeps the author's exact pretraining recipe (in12k_ft_in1k_384) and small's
    # stage widths, differing only in stage-3 depth (9 blocks vs 27): a clean ablation.
    # base has no in12k_ft_in1k_384 weights; fb_in22k_ft_in1k_384 is the nearest recipe.
    def _backbone(name):
        c = deepcopy(base)
    ...
    return [
        ...
        # Two follow-ups to cnx_fastvit_kd_small, same teacher (ConvNeXt-small), same settings.
        # sa24: twice sa12's depth (20.4M backbone vs 10.4M).  Reparameterised it runs in ~22 ms
        # in the U-Net, the same as sa12 un-reparameterised, so it buys capacity at today's cost.
        # t12: sa12 with its one attention stage replaced by RepMixer convs -- the clean ablation of
        # whether FastViT's attention contributes anything (it costs only ~1 ms of ~19).
        ("cnx_fastvit_sa24_kd_small", _kd_ens("hf_hub:timm/fastvit_sa24.apple_dist_in1k",
                                              teachers=["results/0801_V2_ep150"])),
        ("cnx_fastvit_t12_kd_small", _kd_ens("hf_hub:timm/fastvit_t12.apple_dist_in1k",
                                             teachers=["results/0801_V2_ep150"])),
...
SEQ_TRAIN_CFGS = _validate_seq_train_cfgs(
    list(make_seq_requested_ablation_cfgs()) + make_bilzard_cfgs()
)
