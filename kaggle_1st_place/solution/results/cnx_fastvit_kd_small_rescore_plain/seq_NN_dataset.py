...
from seq_NN_master_typewell import build_master_typewell_bank


def uses_master_typewell(cfg):
    """True when a recipe asks for master-series re-skinning during simulation."""

    sim_cfg = getattr(cfg, "sim_cfg", {}) or {}
    return float(sim_cfg.get("reskin", {}).get("apply_prob", 0.0)) > 0.0
...
def md_phase_shift(cfg):
    """Sub-bin offset of the MD downsampling grid.

    The pipeline averages ``cfg.downsample`` raw rows into one column, and the
    column cut lines are pinned to the prediction-start row.  Where exactly the
    cuts fall is arbitrary at a resolution finer than one column, and the box
    average aliases whatever sits inside a column, so the input tensor -- and
    therefore the prediction -- depends on the cut phase.  Shifting the window
    boundary left by ``phase`` rows moves every cut line by the same amount,
    which is what the MD-phase test-time augmentation averages over.
    """

    downsample = int(cfg.downsample)
    if downsample <= 0:
        raise ValueError(f"cfg.downsample must be positive, got {downsample}")
    phase = int(getattr(cfg, "md_phase_shift", 0) or 0) % downsample
    if phase < 0:
        phase += downsample
    return phase
...
def _build_raw_window_native(horizontal, last_seen_idx, cfg):
    ...
    # The prediction-start boundary normally sits on the bin edge at
    # ``prefix_len``.  Moving it back by ``phase`` rows re-phases every
    # downsampling cut line without touching which rows are known.
    phase = md_phase_shift(cfg)
    boundary = prefix_len - phase
    raw = {
    ...
    prefix_start = max(0, last_seen_idx + 1 - boundary)
    prefix_size = last_seen_idx + 1 - prefix_start
    prefix_dest_start = boundary - prefix_size
    ...
    if prefix_size > 0:
        ...
        dst = slice(prefix_dest_start, boundary)
    ...
    if suffix_size > 0:
        ...
        dst = slice(boundary, boundary + suffix_size)
        for name, arr in source.items():
            raw[name][dst] = arr[src]
        raw_has_row[dst] = True
...
def _make_common_item(well_data, cfg, training):
    ...
    meta = {
        ...
        "md_phase": int(anchor.get("md_phase", 0)),
...
def _prefix_direct_gr_rmse_after_downsample(horizontal, typewell, cfg):
    ...
    boundary = cfg.prefix_len - md_phase_shift(cfg)
    prefix_start = max(0, last_seen_idx + 1 - boundary)
    prefix_size = last_seen_idx + 1 - prefix_start
    prefix_dest_start = boundary - prefix_size
    src = slice(prefix_start, last_seen_idx + 1)
...
class TrendSampler:
    ...
    def __init__(self, well_data_seq, z_shift_cfg=None):
        ...
        # Trajectory mixup, from the 2nd-place synthesis: blending two real
        # suffix paths deforms the shape while staying inside the convex hull
        # of shapes the field actually produced, which free-form curves do not.
        self.mixup_prob = float(z_shift_cfg.get("trend_mixup_prob", 0.0))
        self.mixup_lam_range = z_shift_cfg.get("trend_mixup_lam_range", (0.3, 0.7))
        self.TVT_rel_pool = []
    ...
    def _sample_single_trend(self, L, pool, idx=None):
        if idx is None:
    ...
    def get(self, L, idx=None, well_id=None):
        ...
        trend = self._sample_single_trend(L, pool, idx=idx)
        if (
            self.mixup_prob > 0.0
            and len(pool) > 1
            and random.random() < self.mixup_prob
        ):
            other = self._sample_single_trend(L, pool, idx=None)
            low, high = self.mixup_lam_range
            lam = np.float32(np.random.uniform(float(low), float(high)))
            trend = (lam * trend + (np.float32(1.0) - lam) * other).astype("float32")
        return trend
...
class Simulator:
    def __init__(self,well_data_seq, sim_cfg={},aug_cfg={}, cfg=None):
        ...
        self.master_typewell_bank=None
        reskin_cfg=sim_cfg.get('reskin',{})
        if float(reskin_cfg.get('apply_prob',0.0))>0.0:
            self.master_typewell_bank=build_master_typewell_bank(
                well_data_seq,
                reskin_cfg,
                log=print,
            )
        ...
        if 'real_block' in z_noise_modes or reskin_cfg.get('noise_mode')=='real_block':
    ...
    def reskin_typewell(self, well_data, cfg=None):
        '''
        Re-place the current trajectory into another typewell system.

        This is the 2nd-place solution's re-skinning step.  The trajectory shape
        is kept exactly as it is; what changes is which stratigraphic column the
        well is drilled through, and at what depth inside that column.  The
        depth is not uniform-random: the quantile the well occupies within the
        drilling band of its own system -- the levels wells of that system are
        actually steered through -- is carried over to the target system's band,
        so the placement stays somewhere the field really drills.

        ``z`` is a fact about the trajectory and is left untouched, so the
        placement shift lands entirely on ``z_layer = TVT + z``.  Every absolute
        surface sidecar is shifted with it, which keeps the geo prior aligned.
        '''
        cfg=self.sim_cfg.get('reskin',{}) if cfg is None else cfg
        bank=self.master_typewell_bank
        if bank is None or bank.system_count<=1:
            return well_data
        horizontal=well_data['horizontal']
        TVT=horizontal['TVT']
        ...
        if finite_tvt.sum()<2:
            return well_data
        tvt_lo=float(np.min(TVT[finite_tvt]))
        tvt_hi=float(np.max(TVT[finite_tvt]))
        src_system=bank.system_of_well.get(well_data['well_id'])
        if src_system is None:
            return well_data

        default_margin=float(getattr(self.cfg,'typewell_window',100.0)) if self.cfg is not None else 100.0
        configured_margin=cfg.get('crop_margin_ft')
        margin=default_margin if configured_margin is None else float(configured_margin)
        jitter=float(cfg.get('quantile_jitter',0.1))
        attempts=int(cfg.get('max_attempts',8))
        src_lo,src_hi=bank.band(src_system)
        center=0.5*(tvt_lo+tvt_hi)
        src_span=max(src_hi-src_lo,1e-3)
        quantile=float(np.clip((center-src_lo)/src_span,0.0,1.0))

        crop=None
        delta=0.0
        for _ in range(max(1,attempts)):
            dst_system=bank.sample_system_index()
            dst_lo,dst_hi=bank.band(dst_system)
            target_q=float(np.clip(quantile+np.random.uniform(-jitter,jitter),0.0,1.0))
            candidate_delta=(dst_lo+target_q*(dst_hi-dst_lo))-center
            candidate=bank.crop(
                dst_system,
                tvt_lo+candidate_delta-margin,
                tvt_hi+candidate_delta+margin,
            )
        ...
        residual=(GR-old_matched_GR).astype('float32')
        TVT_new=(TVT+np.float32(delta)).astype('float32')
        new_matched_GR=np.interp(
            TVT_new,
            crop['TVT'],
            crop['GR'],
        ...
        noise_mode=str(cfg.get('noise_mode','copy'))
        if noise_mode=='copy':
            GR_noise=_fill_nan_linear(residual)
        elif noise_mode=='real_block':
            # Paste a real well's residual sequence, which carries both the
            # well-specific GR signature and its autocorrelated remainder.
            GR_noise=residual.copy()
            suffix_cond=np.isnan(horizontal['TVT_input'])
            if suffix_cond.any():
                GR_noise[suffix_cond]=self._sample_real_block_noise(
                    well_data,
                    residual[suffix_cond],
                )
            GR_noise=_fill_nan_linear(GR_noise)
        else:
            raise ValueError(f"unknown reskin noise_mode: {noise_mode!r}")
        GR_sim=(new_matched_GR+GR_noise).astype('float32')
        if bool(cfg.get('same_nan',True)):
            GR_sim[np.isnan(GR)]=np.nan

        TVT_input=horizontal['TVT_input']
        TVT_input_new=TVT_input.copy()
        finite_input=np.isfinite(TVT_input_new)
        TVT_input_new[finite_input]=TVT_input_new[finite_input]+np.float32(delta)

        horizontal['TVT']=TVT_new
        horizontal['TVT_input']=TVT_input_new
        horizontal['GR']=GR_sim
        for surface_name in ('geo_s_rel_prior_abs','S_avg'):
            if surface_name in horizontal:
                horizontal[surface_name]=(
                    horizontal[surface_name]+np.float32(delta)
                ).astype('float32')
        well_data['typewell']={'TVT': crop['TVT'], 'GR': crop['GR']}
        if 'typewell_raw' in well_data:
            well_data['typewell_raw']={'TVT': crop['TVT'], 'GR': crop['GR']}
        return well_data
    ...
    def process(self,well_data):
        ...
        reskin_cfg=self.sim_cfg.get('reskin',{})
        if random.random()<float(reskin_cfg.get('apply_prob',0.0)):
            well_data=self.reskin_typewell(well_data,cfg=reskin_cfg)
...
class SeqUNetDataset(Dataset):
    ...
    def _load_well_data(self, well_id):
        ...
        master_raw_typewell = None
        if self.simulation and uses_master_typewell(self.cfg):
            # The master-series consolidation compares typewells to each other,
            # so it has to see them before the per-well GR calibration below
            # pulls each one toward its own seen prefix.
            master_order = np.argsort(typewell_df["TVT"].to_numpy(dtype="float32"))
            master_raw_typewell = {
                "TVT": typewell_df["TVT"].to_numpy(dtype="float32", copy=True)[master_order],
                "GR": typewell_df["GR"].to_numpy(dtype="float32", copy=True)[master_order],
            }
        ...
        if master_raw_typewell is not None:
            well_data["typewell_master_raw"] = master_raw_typewell
