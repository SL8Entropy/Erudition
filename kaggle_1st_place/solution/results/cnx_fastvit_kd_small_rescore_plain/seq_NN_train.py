...
from seq_NN_models import (
    SeqBiConvGRUModel,
    SeqTransformerUNetModel,
    SeqTwoStageUNetModel,
    SeqUNet2DModel,
)
from seq_NN_cfg import (
    ...
    UNET_MODE,
    resolve_finetune_disabled_augs,
)
...
def resolve_backbone_frozen_epochs(cfg):
    """Return the active pretrained-backbone freeze window, if applicable."""
    frozen_epochs = getattr(cfg, "backbone_frozen_epochs", None)
    if frozen_epochs is None:
        return None
    frozen_epochs = int(frozen_epochs)
    if frozen_epochs < 0:
        raise ValueError(
            "backbone_frozen_epochs must be non-negative or None, "
            f"got {frozen_epochs}"
        )

    model_name = getattr(cfg, "model_name", UNET_MODE)
    if model_name == TRF_UNET_MODE:
        return frozen_epochs
    if model_name not in {UNET_MODE, TWO_STAGE_UNET_MODE}:
        return None
    model_cfg = getattr(cfg, "model_cfg", None)
    if model_cfg is None:
        model_cfg = (
            getattr(cfg, "two_stage_unet_cfg", {})
            if model_name == TWO_STAGE_UNET_MODE
            else getattr(cfg, "unet_cfg", {})
        )
    model_cfg = model_cfg or {}
    if str(model_cfg.get("unet_arch", "unet")) != "convnext_small":
        return None
    return frozen_epochs


def _convnext_backbone_stages(model):
    source_model = source_model_for_train_ops(model)
    if not isinstance(source_model, SeqUNet2DModel):
        return None
    if getattr(source_model, "unet_arch", None) != "convnext_small":
        return None
    stages = getattr(getattr(source_model, "unet", None), "backbone_stages", None)
    if stages is None:
        raise ValueError("ConvNeXt U-Net model is missing its backbone stages")
    return stages


def _pretrained_backbone_module(model):
    source_model = source_model_for_train_ops(model)
    if isinstance(source_model, SeqTransformerUNetModel):
        backbone = getattr(getattr(source_model, "unet", None), "backbone", None)
        if backbone is None:
            raise ValueError("Transformer U-Net model is missing its timm backbone")
        return backbone
    return _convnext_backbone_stages(model)


def _set_pretrained_backbone_requires_grad(model, enabled):
    backbone = _pretrained_backbone_module(model)
    if backbone is None:
        return False
...
def _muon_param_groups_from_param_groups(param_groups):
    """Split layer-wise groups into Muon and AdamW updates without losing LR scales."""
    groups = []
    for base_group in param_groups:
        muon_params = []
        adamw_params = []
        for param in base_group["params"]:
            if param.ndim >= 2:
                muon_params.append(param)
            else:
                adamw_params.append(param)
        group_kwargs = {key: value for key, value in base_group.items() if key != "params"}
        if muon_params:
            muon_group = dict(group_kwargs)
            muon_group["params"] = muon_params
            muon_group["use_muon"] = True
            groups.append(muon_group)
        if adamw_params:
            adamw_group = dict(group_kwargs)
            adamw_group["params"] = adamw_params
            adamw_group["use_muon"] = False
            groups.append(adamw_group)
    if not any(group.get("use_muon", False) for group in groups):
        raise ValueError("optimizer='muon' found no ndim>=2 parameters for Muon updates")
    return groups


def _convnext_layerwise_param_groups(model, named_params, *, lr, lr_scale, layer_decay):
    """Build a base-rate custom group and one decayed group per ConvNeXt stage."""
    layer_decay = float(layer_decay)
    if not math.isfinite(layer_decay) or not 0.0 < layer_decay <= 1.0:
        raise ValueError(f"layer_decay must be finite and in (0, 1], got {layer_decay}")
    unet_model = source_model_for_train_ops(model)
    unet = getattr(unet_model, "unet", None)
    stages = getattr(unet, "backbone_stages", None)
    if stages is None:
        return None
    ...
    stage_params = [[] for _ in range(stage_count)]
    custom_params = []
    stage_prefix = "unet.backbone_stages."
    for name, param in named_params:
        if not param.requires_grad:
            continue
        stage_index = None
        if name.startswith(stage_prefix):
            stage_name = name[len(stage_prefix) :].split(".", 1)[0]
            if stage_name.isdigit():
                stage_index = int(stage_name)
        if stage_index is None or not 0 <= stage_index < stage_count:
            custom_params.append(param)
        else:
            stage_params[stage_index].append(param)

    if not custom_params and not any(stage_params):
        return None
    if any(not params for params in stage_params):
        empty_stages = [index for index, params in enumerate(stage_params) if not params]
        raise ValueError(f"ConvNeXt layer-wise LR found no trainable parameters for stages={empty_stages}")

    base_lr = float(lr) * float(lr_scale)
    groups = []
    if custom_params:
        groups.append(
            {
                ...
                "lr": base_lr,
                "layerwise_lr": True,
                "layerwise_name": "custom",
            }
        )
    # Earlier/shallow stages receive the smallest rate; the final stage gets
    # lr * layer_decay, while wrapper stem/decoder/head layers stay at base_lr.
    for stage_index, params in enumerate(stage_params):
        stage_lr = base_lr * layer_decay ** (stage_count - stage_index)
        groups.append(
            {
                "params": params,
                "lr": stage_lr,
                "layerwise_lr": True,
                "layerwise_name": f"convnext_stage_{stage_index}",
            }
        )
    return groups


def _trf_layerwise_param_groups(model, named_params, *, lr, lr_scale, layer_decay):
    """Build one decayed group for the timm backbone and one base-rate custom group."""
    layer_decay = float(layer_decay)
    if not math.isfinite(layer_decay) or not 0.0 < layer_decay <= 1.0:
        raise ValueError(f"layer_decay must be finite and in (0, 1], got {layer_decay}")
    source_model = source_model_for_train_ops(model)
    if not isinstance(source_model, SeqTransformerUNetModel):
        return None
    base_lr = float(lr) * float(lr_scale)
    backbone_params = []
    custom_params = []
    for name, param in named_params:
        if not param.requires_grad:
            continue
        if name.startswith("unet.backbone."):
            backbone_params.append(param)
        else:
            custom_params.append(param)
    if not backbone_params:
        raise ValueError("Transformer layer-wise LR found no backbone parameters")
    groups = []
    if custom_params:
        groups.append(
            {
...
def _make_optimizer(model, cfg, lr_scale=1.0):
    ...
    layerwise_groups = None
    if bool(optimizer_cfg.get("layer_wise_lr", False)):
        layerwise_kwargs = {
            "lr": optimizer_cfg["lr"],
            "lr_scale": lr_scale,
            "layer_decay": float(optimizer_cfg.get("layer_decay", 0.8)),
        }
        layerwise_groups = _trf_layerwise_param_groups(
            model,
            named_params,
            **layerwise_kwargs,
        )
        if layerwise_groups is None:
            layerwise_groups = _convnext_layerwise_param_groups(
                model,
                named_params,
                **layerwise_kwargs,
            )
    if name == "adamw":
        optimizer_params = layerwise_groups if layerwise_groups is not None else params
        return torch.optim.AdamW(optimizer_params, **common_kwargs)
    if name == "radam":
        ...
        optimizer_params = layerwise_groups if layerwise_groups is not None else params
        return torch.optim.RAdam(optimizer_params, **radam_kwargs)
    if name == "muon":
        if layerwise_groups is None:
            muon_groups = _muon_param_groups_from_named_params(named_params)
        else:
            muon_groups = _muon_param_groups_from_param_groups(layerwise_groups)
...
def _make_scheduler(
    ...
    *,
    ...
    layerwise = any(bool(group.get("layerwise_lr", False)) for group in optimizer.param_groups)
    ...
    if scheduler_name == "onecycle":
        cycle_max_lr = max_lr
        if layerwise:
            # OneCycleLR accepts a per-group max_lr list. Using each group's
            # initial rate keeps the stage decay ratios throughout the cycle.
            cycle_max_lr = [float(group["lr"]) for group in optimizer.param_groups]
...
def model_gr_corr_weight_enabled(cfg):
    return bool(getattr(cfg, "gr_corr_weight", False))


def model_gr_corr_weight_params(cfg):
    power = float(getattr(cfg, "gr_corr_weight_p", 1.0))
    if not math.isfinite(power) or power <= 0.0:
        raise ValueError(f"gr_corr_weight_p must be finite and positive, got {power}")

    raw_window = getattr(cfg, "gr_corr_weight_smooth_window", 5)
    try:
        smooth_window = int(raw_window)
    except (TypeError, ValueError, OverflowError):
        smooth_window = 0
    if isinstance(raw_window, bool) or smooth_window != raw_window or smooth_window <= 0:
        raise ValueError(
            "gr_corr_weight_smooth_window must be a positive integer, "
            f"got {raw_window}"
        )

    smooth_factor = float(getattr(cfg, "gr_corr_weight_smooth_factor", 0.5))
    if not math.isfinite(smooth_factor) or smooth_factor < 0.0:
        raise ValueError(
            "gr_corr_weight_smooth_factor must be finite and non-negative, "
            f"got {smooth_factor}"
        )
    return power, smooth_window, smooth_factor


def _masked_sequence_smooth(values, valid_mask, window):
    if window == 1:
        return values
    pad_left = (window - 1) // 2
    pad_right = window // 2
    mask_float = valid_mask.to(dtype=values.dtype)
    value_sum = F.avg_pool1d(
        F.pad((values * mask_float).unsqueeze(1), (pad_left, pad_right)),
        kernel_size=window,
        stride=1,
    ).squeeze(1) * window
    valid_count = F.avg_pool1d(
        F.pad(mask_float.unsqueeze(1), (pad_left, pad_right)),
        kernel_size=window,
        stride=1,
    ).squeeze(1) * window
    return value_sum / valid_count.clamp_min(1.0)


def model_gr_corr_loss_weight(gr, gr_mask, target_mask, cfg):
    """Build missing-aware GR-leverage weights with mean one per sequence."""
    power, smooth_window, smooth_factor = model_gr_corr_weight_params(cfg)
    uniform_base = 0.01
    gr = gr.to(dtype=torch.float32)
    target_mask = target_mask.to(device=gr.device, dtype=torch.bool)
    gr_mask = gr_mask.to(device=gr.device, dtype=torch.bool)
    valid_gr = target_mask & gr_mask & torch.isfinite(gr)
    valid_count = valid_gr.sum(dim=1, keepdim=True)

    safe_valid_count = valid_count.clamp_min(1).to(dtype=gr.dtype)
    masked_gr = torch.where(valid_gr, gr, torch.zeros_like(gr))
    gr_mean = masked_gr.sum(dim=1, keepdim=True) / safe_valid_count
    centered_gr = torch.where(valid_gr, gr - gr_mean, torch.zeros_like(gr))
    centered_power = centered_gr.abs().pow(power)
    centered_power_mean = centered_power.sum(dim=1, keepdim=True) / safe_valid_count

    smoothed_power = _masked_sequence_smooth(
        centered_power,
        valid_gr,
        smooth_window,
    )
    raw_weight = smoothed_power + smooth_factor * centered_power_mean
    raw_weight = torch.where(target_mask, raw_weight, torch.zeros_like(raw_weight))
    target_count = target_mask.sum(dim=1, keepdim=True).clamp_min(1).to(dtype=gr.dtype)
    mean_weight = raw_weight.sum(dim=1, keepdim=True) / target_count

    informative = (
        (valid_count >= 2)
        & torch.isfinite(mean_weight)
        & (mean_weight > 1e-8)
    )
    safe_mean_weight = torch.where(informative, mean_weight, torch.ones_like(mean_weight))
    signal_weight = raw_weight / safe_mean_weight
    weight = torch.where(
        informative,
        uniform_base + (1.0 - uniform_base) * signal_weight,
        torch.ones_like(signal_weight),
    )
    return torch.where(target_mask, weight, torch.zeros_like(weight))


def model_sequence_loss_weight(
    pred,
    target,
    target_mask,
    cfg,
    ...
    if model_gr_corr_weight_enabled(cfg):
        if gr is None or gr_mask is None:
            raise ValueError(
                "gr_corr_weight requires aux targets 'gr_corr_weight_gr' and "
                "'gr_corr_weight_mask'"
            )
        gr_corr_weight = model_gr_corr_loss_weight(gr, gr_mask, target_mask, cfg)
        loss_weight = gr_corr_weight if loss_weight is None else loss_weight * gr_corr_weight
...
def masked_typewell_cdf_loss(logits, target_probs, target_mask, loss_weight=None, loss_scale_weight=None):
    """Compare ordered Typewell-axis CDFs using a normalized squared score."""
    if target_probs is None:
        raise ValueError("CDF loss requires typewell target probabilities")
    prob = F.softmax(logits.float(), dim=-1)
    target_probs = target_probs.to(device=prob.device, dtype=prob.dtype)
    if target_probs.shape != prob.shape:
        raise ValueError(
            "CDF loss logits and target probabilities must have matching shapes: "
            f"logits={tuple(prob.shape)}, target_probs={tuple(target_probs.shape)}"
        )
    pred_cdf = prob.cumsum(dim=-1)
    target_cdf = target_probs.cumsum(dim=-1)
    per_bin_loss = (pred_cdf - target_cdf).square().mean(dim=-1)
    return _weighted_masked_mean(per_bin_loss, target_mask, loss_weight, loss_scale_weight)
...
def _unet_loss_single(
    ...
    ) and "alignment_logits" not in extra:
    ...
    if (
        ...
        sequence_weight = model_sequence_loss_weight(
            pred,
            target,
            target_mask,
            cfg,
            gr=aux_targets.get("gr_corr_weight_gr"),
            gr_mask=aux_targets.get("gr_corr_weight_mask"),
        )
    ...
    if weights["offset"] != 0.0:
        active_model_cfg = getattr(cfg, "model_cfg", None)
        if active_model_cfg is None:
            active_model_cfg = getattr(cfg, "unet_cfg", {})
        offset_loss = masked_offset_huber_loss(
            ...
            loss_weight=sequence_weight,
            loss_scale_weight=loss_scale_weight,
            detach_logits=bool(active_model_cfg.get("offset_detach_logits_head_in_loss", True)),
        )
    ...
    if weights["CDF"] != 0.0:
        cdf_loss = masked_typewell_cdf_loss(
            extra["alignment_logits"],
            typewell_target_probs,
            target_mask,
            loss_weight=sequence_weight,
        ...
        weighted = weights["CDF"] * cdf_loss
        loss = weighted if loss is None else loss + weighted
        if return_details:
            details["CDF"] = cdf_loss
...
def _deep_sup_stage_weights(cfg, stage_count):
    """Return auxiliary-stage weights in model output order.

    The model stores stages from deepest to shallowest, with the last entry
    nearest the final full-resolution head.  Consequently stage ``i`` gets
    ``decay ** (stage_count - i)`` and the closest stage gets one power of
    decay, matching ``final + decay * stage[-1] + ...``.
    """
    ...
    model_cfg = getattr(cfg, "model_cfg", None)
    if model_cfg is None:
        model_cfg = getattr(cfg, "unet_cfg", {})
    deep_sup = bool(model_cfg.get("deep_sup", False))
    if not deep_sup:
        return ()
    decay = float(model_cfg.get("deep_sup_stage_decay", 0.5))
    if not math.isfinite(decay) or not 0.0 <= decay <= 1.0:
        raise ValueError(
            "deep_sup_stage_decay must be finite and in [0, 1], "
            f"got {decay}"
        )
    return tuple(decay ** power for power in range(stage_count, 0, -1))


def _unpack_deep_sup_stage(stage_output):
    if isinstance(stage_output, dict):
        if "pred" not in stage_output:
            raise ValueError("deep supervision stage output must contain 'pred'")
        pred = stage_output["pred"]
        nested_extra = stage_output.get("extra")
        if nested_extra is not None:
            if not isinstance(nested_extra, dict):
                raise TypeError("deep supervision stage extra output must be a dict")
            return pred, nested_extra
        extra = {key: value for key, value in stage_output.items() if key != "pred"}
        return pred, extra
    if isinstance(stage_output, (tuple, list)) and len(stage_output) == 2:
        pred, extra = stage_output
        if extra is None:
            extra = {}
        ...
        return pred, extra
    raise TypeError(
        "deep supervision stage output must be a dict or a (pred, extra) pair"
    )


def _resize_deep_sup_stage_tensor(value, target_shape):
    """Resize one native-resolution stage tensor to the final loss grid."""
    if value is None or not torch.is_tensor(value):
        return value
    target_rows = int(target_shape[0])
    target_typewell = None if target_shape[1] is None else int(target_shape[1])
    if value.ndim == 2:
        if value.shape[1] == target_rows:
            return value
    ...
    if value.ndim == 3:
        if target_typewell is None:
            return value
        if value.shape[-2:] == (target_rows, target_typewell):
            return value
        return F.interpolate(
            value[:, None, :, :],
            size=(target_rows, target_typewell),
            mode="bilinear",
            align_corners=False,
        ).squeeze(1)
    raise ValueError(
        "deep supervision tensors must be [batch, rows] or [batch, rows, typewell], "
...
def unet_loss(
    ...
    """Compute final U-Net loss plus optionally weighted deep-supervision stages."""
    if extra is None:
        raise ValueError("unet requires model extra_return=True")

    final_loss, final_details = _unet_loss_single(
        pred,
        target,
        target_mask,
        typewell_target_probs,
    ...
    details = dict(final_details)
    stage_outputs = extra.get("deep_sup_stages", ())
    ...
    stage_weights = _deep_sup_stage_weights(cfg, len(stage_outputs))
    if len(stage_weights) != len(stage_outputs):
        raise ValueError(
            "deep supervision stages were returned but cfg.model_cfg['deep_sup'] is disabled"
        )
    loss = final_loss
    for stage_output, stage_weight in zip(stage_outputs, stage_weights):
        if stage_weight == 0.0:
            continue
        stage_pred, stage_extra = _unpack_deep_sup_stage(stage_output)
        target_typewell = None
        if torch.is_tensor(typewell_target_probs):
            target_typewell = typewell_target_probs.shape[-1]
        elif hasattr(cfg, "typewell_len"):
            target_typewell = int(cfg.typewell_len)
        target_shape = (target.shape[1], target_typewell)
        stage_pred = _resize_deep_sup_stage_tensor(stage_pred, target_shape)
        stage_extra = {
            key: _resize_deep_sup_stage_tensor(value, target_shape)
            for key, value in stage_extra.items()
        }
        stage_loss, stage_details = _unet_loss_single(
            stage_pred,
            target,
            target_mask,
            typewell_target_probs,
        ...
        for name, value in stage_details.items():
            details[name] = details.get(name, value.new_zeros(())) + stage_weight * value
    if return_details:
        return loss, details
    return loss
...
def validate_seq_target_support(cfg):
    ...
    model_cfg = getattr(cfg, "model_cfg", None)
    if model_cfg is None:
        model_cfg = getattr(cfg, "unet_cfg", None)
    if not isinstance(model_cfg, dict):
        raise ValueError(f"{model_name} model_cfg must be a dictionary")
    if model_name == TRF_UNET_MODE:
        backbone_name = str(model_cfg.get("backbone_name", ""))
        if backbone_name not in TRF_UNET_BACKBONES:
            raise ValueError(
                f"unknown trf_unet backbone_name={backbone_name!r}; expected one of "
                f"{sorted(TRF_UNET_BACKBONES)}"
            )
        pretrained = model_cfg.get("pretrained", None)
        if pretrained is not None and not isinstance(pretrained, bool):
            raise ValueError("trf_unet pretrained must be boolean or None")
        norm_replace_keys = sorted(
            key for key in model_cfg if key.startswith("unet_convnext_")
        )
        if norm_replace_keys:
            raise ValueError(
                "trf_unet keeps native backbone normalization and does not accept "
                f"ConvNeXt normalization-replacement settings: {norm_replace_keys}"
            )
        emb_dim = int(model_cfg.get("emb_dim", 0))
        if emb_dim <= 0:
            raise ValueError(f"trf_unet emb_dim must be positive, got {emb_dim}")
        res_kernel_size = int(model_cfg.get("res_kernel_size", 0))
        if res_kernel_size <= 0 or res_kernel_size % 2 == 0:
            raise ValueError(
                "trf_unet res_kernel_size must be a positive odd integer, "
                f"got {res_kernel_size}"
            )
        for field_name in ("pad_value", "channel_mask_value"):
            field_value = float(model_cfg.get(field_name, 0.0))
            if not math.isfinite(field_value):
                raise ValueError(f"trf_unet {field_name} must be finite, got {field_value}")
        for field_name in (
            "dropout",
            "drop_path_rate",
            "channel_mask_prob",
            ...
            if not math.isfinite(field_value) or not 0.0 <= field_value <= 1.0:
                raise ValueError(
                    f"trf_unet {field_name} must be finite and in [0, 1], got {field_value}"
                )
    ...
    unet_arch = str(model_cfg.get("unet_arch", "unet"))
    deep_sup = model_cfg.get("deep_sup", False)
    if not isinstance(deep_sup, bool):
        raise ValueError(f"deep_sup must be boolean, got {deep_sup!r}")
    deep_sup_stage_decay = float(model_cfg.get("deep_sup_stage_decay", 0.5))
    if not math.isfinite(deep_sup_stage_decay) or not 0.0 <= deep_sup_stage_decay <= 1.0:
        raise ValueError(
            "deep_sup_stage_decay must be finite and in [0, 1], "
    ...
    if deep_sup and not (
        ...
        raise ValueError(
            "deep_sup=True is supported only for model_name='trf_unet' or "
            "model_name='unet' with unet_arch='convnext_small'"
        )
    if model_name != TRF_UNET_MODE:
    ...
    if model_name == BIDIR_CONVGRU_MODE:
        ...
        if hidden_dim <= 0 or hidden_dim % 2 != 0:
            raise ValueError(f"bidir_convgru hidden_dim must be a positive even integer, got {hidden_dim}")
        if layers <= 0:
            raise ValueError(f"bidir_convgru layers must be positive, got {layers}")
        if kernel_size <= 0 or kernel_size % 2 == 0:
            raise ValueError(f"bidir_convgru kernel_size must be a positive odd integer, got {kernel_size}")
        if not 0.0 <= dropout <= 1.0:
            raise ValueError(f"bidir_convgru dropout must be in [0, 1], got {dropout}")
        if norm not in {"BN", "IN", "LN"}:
            raise ValueError(f"bidir_convgru norm must be one of ('BN', 'IN', 'LN'), got {norm!r}")
        if act not in {"silu", "relu"}:
            raise ValueError(f"bidir_convgru act must be one of ('silu', 'relu'), got {act!r}")
    if model_name == TWO_STAGE_UNET_MODE:
    ...
    model_gr_corr_weight_params(cfg)
...
def resolve_md_phases(cfg):
    """MD-phase offsets, in raw rows, to average over at inference time.

    ``cfg.md_phase_tta`` accepts an integer count -- the phases are then spread
    evenly across one ``cfg.downsample`` column -- or an explicit sequence of
    row offsets.  Anything that reduces to a single phase leaves inference
    exactly as archived.
    """

    downsample = int(cfg.downsample)
    spec = getattr(cfg, "md_phase_tta", None)
    if spec is None:
        return (0,)
    if isinstance(spec, bool):
        raise TypeError("cfg.md_phase_tta must be an int count or a sequence of row offsets")
    if isinstance(spec, int):
        ...
        if count > downsample:
            raise ValueError(
                f"cfg.md_phase_tta={count} exceeds cfg.downsample={downsample}; "
                "there are only that many distinct column grids"
            )
        step = downsample / float(count)
        spec = [int(round(index * step)) for index in range(count)]
    phases = tuple(dict.fromkeys(int(value) % downsample for value in spec))
    if len(phases) == 0:
        return (0,)
    return phases


def md_phase_data_cfg(cfg, phase):
    """Shallow config clone that builds items on the ``phase``-shifted grid."""

    phase_cfg = copy.copy(cfg)
    phase_cfg.md_phase_shift = int(phase)
    return phase_cfg


def _md_phase_loader(base_loader, cfg, phase, seed):
    """Re-view an already-loaded inference dataset on a shifted column grid.

    The well CSVs are the expensive part of building a loader and they do not
    depend on the phase, so the dataset object is shallow-copied and only its
    config is swapped.  Workers are never persistent here: they are forked per
    phase and must not outlive it.
    """

    dataset = copy.copy(base_loader.dataset)
    if getattr(dataset, "simulation", False):
        raise ValueError("MD-phase TTA is an inference-time view; the dataset must not simulate")
    dataset.cfg = md_phase_data_cfg(cfg, phase)
    generator = torch.Generator()
    generator.manual_seed(int(seed))
    kwargs = {}
    if cfg.num_workers > 0:
...
MD_PHASE_TTA_AVERAGED_COLUMNS = ("TVT_pred", "target_pred", "pred")


def predict_with_md_phase_tta(models, loader, cfg, *, include_target, apply_smooth, log=print):
    """Average model predictions over several MD column-grid phases.

    Bin-space predictions from different phases live on different grids, so the
    averaging happens after each phase has been expanded back onto the original
    suffix rows.  Row order is identical across phases -- the same wells in the
    same loader order, and ``suffix_len`` does not depend on the phase -- which
    is asserted rather than assumed.
    """

    model_list = list(models.values()) if isinstance(models, dict) else list(models)
    if len(model_list) == 0:
        raise ValueError("predict_with_md_phase_tta requires at least one model")
    phases = resolve_md_phases(cfg)
    base_phase = int(getattr(loader.dataset.cfg, "md_phase_shift", 0) or 0)
    log(f"MD-phase TTA: phases={phases} models={len(model_list):,}")

    base_df = None
    sums = None
    for phase in phases:
        phase_cfg = md_phase_data_cfg(cfg, phase)
        if phase == base_phase:
            phase_loader = loader
        else:
            phase_loader = _md_phase_loader(loader, cfg, phase, seed=cfg.seed + 900000 + phase)
        pred_sum = None
        metas = None
        for model in model_list:
            pred, metas = predict_one_model(model, phase_loader, phase_cfg, return_meta=True)
            contribution = pred / float(len(model_list))
            pred_sum = contribution if pred_sum is None else pred_sum + contribution
        phase_df = make_prediction_df(
            pred_sum,
            metas,
            phase_cfg,
        ...
        if base_df is None:
            base_df = phase_df
            sums = {
                column: phase_df[column].to_numpy(dtype=np.float64, copy=True)
                for column in MD_PHASE_TTA_AVERAGED_COLUMNS
                if column in phase_df.columns
            }
            continue
        if len(phase_df) != len(base_df):
            raise ValueError(
                f"MD-phase TTA row-count mismatch at phase={phase}: "
                f"{len(phase_df)} vs {len(base_df)}"
            )
        if not phase_df["well_id"].equals(base_df["well_id"]) or not phase_df[
            "submit_index"
        ].equals(base_df["submit_index"]):
            raise ValueError(f"MD-phase TTA row order changed at phase={phase}")
        for column, total in sums.items():
            total += phase_df[column].to_numpy(dtype=np.float64, copy=False)

    out = base_df
    for column, total in sums.items():
        out[column] = (total / float(len(phases))).astype("float32")
    if apply_smooth:
        out = apply_pred_sg_smooth(out, cfg)
    return out


def make_finetune_data_cfg(cfg):
    finetune_cfg = copy.copy(cfg)
    finetune_cfg.sim_cfg = copy.deepcopy(cfg.sim_cfg)
    finetune_cfg.aug_cfg = copy.deepcopy(cfg.aug_cfg)
    for aug_name in resolve_finetune_disabled_augs(cfg):
        for aug_group in (finetune_cfg.sim_cfg, finetune_cfg.aug_cfg):
            if aug_name in aug_group:
                aug_group[aug_name]["apply_prob"] = 0.0
    return finetune_cfg
...
def make_model(cfg):
    ...
    if model_name == TRF_UNET_MODE:
        if bool(getattr(cfg, "submit_mode", False)):
            pretrained = False
        elif model_cfg.get("pretrained", None) is None:
            pretrained = True
        else:
        ...
        model_cfg["unet_pretrained"] = unet_pretrained
    model_cfg.update(shared_model_kwargs)
    ...
    if model_name == TRF_UNET_MODE:
        return SeqTransformerUNetModel(
            unet_static_in_dim=len(cfg.unet_static_channels),
...
def move_model_to_device(model, cfg):
    ...
    if uses_channels_last_unet(cfg) and not isinstance(source_model_for_train_ops(model), SeqBiConvGRUModel):
...
def _expand_full_size_bin_values(bin_values, meta, cfg):
    ...
    boundary = np.float32(cfg.prefix_len - int(meta.get("md_phase", 0)))
    query = (boundary + dest_pos) / np.float32(cfg.downsample)
...
def expand_target_prediction(bin_pred, meta, cfg):
    ...
    mean, std = cfg.target_stats[UNET_MODE]
    boundary = cfg.prefix_len - int(meta.get("md_phase", 0))
    for raw_idx in range(kept_len):
        bin_idx = (boundary + raw_idx) // cfg.downsample
...
def add_geo_prior_diagnostic_columns(df, geo_prior):
    """Attach inference-safe original-row geo diagnostics for saved analysis only.

    This includes ``geo_prefix_weight_ratio``. These columns are attached after
    inference and are never consumed by the dataset, model, losses, or optimizer.
    """
...
def repeated_oof_average_columns(df):
    base_columns = (
        ...
        "PF_TVT",
        "geo_prior_TVT",
        # Pointwise geo sidecars, including prefix weight, must be averaged
        # before any per-well summary is derived.
        *POST_TRAIN_GEO_DIAGNOSTIC_NAMES,
    )
...
def run_training(model, train_loader, val_loader, cfg, log, fold_name, finetune_loader=None):
    ...
    def run_phase(
        *,
        ...
        configured_backbone_frozen_epochs = getattr(cfg, "backbone_frozen_epochs", None)
        backbone_frozen_epochs = (
            resolve_backbone_frozen_epochs(cfg) if phase_name == "train" else None
        )
        if (
            phase_name == "train"
            and configured_backbone_frozen_epochs is not None
            and backbone_frozen_epochs is None
        ):
            model_cfg = getattr(cfg, "model_cfg", {}) or {}
            log(
                f"{phase_label}: backbone_frozen_epochs disabled for "
                f"model_name={getattr(cfg, 'model_name', UNET_MODE)!r}, "
                f"unet_arch={model_cfg.get('unet_arch', 'unet')!r}"
            )
        pretrained_backbone = _pretrained_backbone_module(model)
        if backbone_frozen_epochs is not None and backbone_frozen_epochs > 0:
            if pretrained_backbone is None:
                raise ValueError(
                    "backbone_frozen_epochs is active but the model has no pretrained backbone"
                )
            backbone_parameter_count = sum(
                parameter.numel() for parameter in pretrained_backbone.parameters()
            )
            log(
                f"{phase_label}: pretrained backbone parameters frozen for epochs "
                f"1..{min(int(epochs), backbone_frozen_epochs)} "
                f"({backbone_parameter_count:,} parameters)"
            )
        # Build the optimizer with every parameter included.  The epoch-level
        # requires_grad toggle below then lets the same optimizer update the
        # backbone as soon as the freeze window ends.
        _set_pretrained_backbone_requires_grad(model, True)
        ...
        for epoch in range(1, int(epochs) + 1):
            ...
            should_freeze_backbone = (
                backbone_frozen_epochs is not None
                and epoch <= backbone_frozen_epochs
            )
            if should_freeze_backbone != backbone_is_frozen:
                _set_pretrained_backbone_requires_grad(model, not should_freeze_backbone)
    ...
    if finetune_epochs > 0:
        ...
        log(
            ...
            f"simulation=True, disabled_augs={list(resolve_finetune_disabled_augs(cfg))}"
        )
        run_phase(
            phase_name="finetune",
...
def reparameterize_for_inference(model, log=None):
    """Collapse train-time multi-branch blocks into single convs before inference.

    MobileOne and FastViT train with parallel conv/BN/identity branches that fold into one conv
    once the weights are fixed (timm's `reparameterize` / `switch_to_deploy`).  The result is the
    same function up to float rounding -- 1.9e-4 relative on the trained FastViT student, 20x below
    the bf16 drift inference already accepts -- and cut that student's U-Net from 18.6 to 14.6 ms.
    Backbones without such blocks (ConvNeXt) are left untouched.  Idempotent.  Returns the number
    of blocks that were still multi-branch.
    """
    from timm.utils.model import reparameterize_model

    pending = [
        m for m in model.modules()
        if (type(m).__name__ == "MobileOneBlock" and getattr(m, "reparam_conv", None) is None)
        or (type(m).__name__ == "RepMixer" and not getattr(m, "inference_mode", False))
    ]
    if not pending:
        return 0
    model.eval()
    ...
    if log is not None:
        log(f"reparameterised {len(pending)} multi-branch blocks for inference")
    return len(pending)
...
def kfold_training(well_ids, cfg, log):
    ...
    for fold, (train_index, val_index) in enumerate(splits):
        ...
        finetune_epochs = 0 if finetune_epochs is None else int(finetune_epochs)
        if finetune_epochs > 0:
            finetune_cfg = make_finetune_data_cfg(cfg)
        ...
        # Score (and save) the collapsed model: it is what a deployment would run.
        if bool(getattr(cfg, "reparam_at_inference", True)):
            reparameterize_for_inference(model, log=log)
        fold_score = None
        fold_well_summary = None
        if val_loader is not None:
            if len(resolve_md_phases(cfg)) > 1:
                # Checkpoint selection during training stays on the archived
                # single-phase view; only the final scored prediction is
                # averaged over phases.
                tmp_raw = predict_with_md_phase_tta(
                    [model],
                    val_loader,
                    cfg,
                    include_target=True,
...
def predict_models(models, well_ids, cfg, train_wells=None, log=print):
    ...
    if len(resolve_md_phases(cfg)) > 1:
        pred_df = predict_with_md_phase_tta(
            models,
            loader,
            cfg,
