...
class CFG:
    ...
    unet_loss_weights = {
        ...
        "dS_penalty": 0.0,
        "unet_GR_RMSE": 0.0,
        # Architecture experiments; both off by default.
        "unimodal": 0.0,
        "iters": 0.0,
    }
    # Learned-width unimodal cost filtering (AcfNet-style).
    unimodal_sigma_bins = 8.0
    unimodal_sigma_min_bins = 1.0
    unimodal_focal_alpha = 1.0
    unimodal_confidence_weight = 0.05
    # Geometric weighting over refinement rounds (RAFT convention).
    iters_loss_gamma = 0.8
    ...
    def _migrate_legacy_names(self):
        ...
        if "unet_loss_weights" in instance_attrs:
            ...
            # ... and the two architecture-experiment terms.
            self.unet_loss_weights.setdefault("unimodal", 0.0)
            self.unet_loss_weights.setdefault("iters", 0.0)
...
def make_arch_cfgs():
    """Four architecture experiments, each isolating one weakness.

    All four fork ``submit_ver_0801_V2`` and change nothing else, so a holdout
    difference is attributable to the named mechanism.  Every added path starts
    numerically inert -- zero-gated branches, a refiner whose first correction
    is zero -- so training begins from exactly the archived behaviour.

    ``arch_v1_unimodal``  the answer is the mean of a distribution over levels,
                          which is wrong when that distribution is bimodal.
                          Replaces the fixed-width alignment target with a
                          learned-width unimodal one plus a confidence head.
    ``arch_v2_axial``     the MD axis needs thousands of feet of context and a
                          conv stack only relays it through depth.  Adds
                          attention along MD at the three coarsest stages.
    ``arch_v3_raft``      the model gets one shot.  Replaces the one-shot
                          expectation with eight rounds of learned correction.
    ``arch_v4_lkconv``    same problem as v2, blunter instrument: a parallel
                          long-along-MD depthwise kernel inside every ConvNeXt
                          block, tapered as the MD axis shortens.
    """

    base = dict(make_seq_requested_ablation_cfgs())["submit_ver_0801_V2"]

    # -- v1: unimodal cost filtering ---------------------------------------
    unimodal_cfg = deepcopy(base)
    alignment_weight = float(unimodal_cfg.unet_loss_weights["alignment"])
    # The unimodal term replaces alignment rather than joining it: they
    # supervise the same logits for the same purpose, and running both would
    # pull the distribution toward two different target shapes.
    unimodal_cfg.unet_loss_weights["alignment"] = 0.0
    unimodal_cfg.unet_loss_weights["unimodal"] = alignment_weight
    unimodal_cfg.unet_cfg["arch_unimodal_confidence"] = True
    unimodal_cfg.set_model_name(UNET_MODE)
    unimodal_cfg.refresh()

    # -- v2: axial attention along MD --------------------------------------
    axial_cfg = deepcopy(base)
    # Stages 1-3 carry MD lengths 87 / 44 / 22 for the archived geometry, so
    # attention there spans the whole hole at a few megabytes.  Stage 0 (173
    # long, 100 wide) is affordable but the most expensive place to put it.
    axial_cfg.unet_cfg["arch_axial_md_stages"] = (1, 2, 3)
    axial_cfg.unet_cfg["arch_axial_md_depth"] = 1
    axial_cfg.unet_cfg["arch_axial_md_heads"] = 4
    axial_cfg.set_model_name(UNET_MODE)
    axial_cfg.refresh()

    # -- v3: iterative refinement ------------------------------------------
    raft_cfg = deepcopy(base)
    regression_weight = float(raft_cfg.unet_loss_weights["regression"])
    # The per-iteration loss already contains the final prediction at full
    # weight, so the plain regression term would double-count it.
    raft_cfg.unet_loss_weights["regression"] = 0.0
    raft_cfg.unet_loss_weights["iters"] = regression_weight
    raft_cfg.unet_cfg["arch_refine_iters"] = 8
    raft_cfg.unet_cfg["arch_refine_hidden"] = 96
    raft_cfg.unet_cfg["arch_refine_context"] = 96
    raft_cfg.unet_cfg["arch_refine_radius"] = 4
    raft_cfg.unet_cfg["arch_refine_dilations"] = (1, 4, 16)
    raft_cfg.set_model_name(UNET_MODE)
    raft_cfg.refresh()

    # -- v4: anisotropic large kernels -------------------------------------
    lkconv_cfg = deepcopy(base)
    # One kernel length per ConvNeXt stage, tapered with the MD axis
    # (173 / 87 / 44 / 22): a 31-long kernel on a 22-column map is just an
    # expensive global average.
    lkconv_cfg.unet_cfg["arch_large_kernel_md"] = (31, 31, 15, 7)
    lkconv_cfg.unet_cfg["arch_large_kernel_level"] = 3
    lkconv_cfg.set_model_name(UNET_MODE)
    lkconv_cfg.refresh()

    return [
        ("arch_v1_unimodal", unimodal_cfg),
        ("arch_v2_axial", axial_cfg),
        ("arch_v3_raft", raft_cfg),
        ("arch_v4_lkconv", lkconv_cfg),
    ]
...
SEQ_TRAIN_CFGS = _validate_seq_train_cfgs(
    list(make_seq_requested_ablation_cfgs()) + make_arch_cfgs()
)
