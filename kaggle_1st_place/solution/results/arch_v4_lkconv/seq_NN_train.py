...
def unimodal_loss_params(cfg):
    return {
        "sigma_bins": float(getattr(cfg, "unimodal_sigma_bins", 8.0)),
        "sigma_min_bins": float(getattr(cfg, "unimodal_sigma_min_bins", 1.0)),
        "focal_alpha": float(getattr(cfg, "unimodal_focal_alpha", 1.0)),
        "confidence_weight": float(getattr(cfg, "unimodal_confidence_weight", 0.05)),
    }


def masked_unimodal_cost_loss(
    logits,
    target,
    target_mask,
    axis,
    ...
    """Force the level distribution to be a single hump around the truth.

    The model answers with the mean of its distribution over candidate levels.
    That is only sound when the distribution is one hump: when repeating rock
    patterns make it bimodal, the mean lands in the empty valley between the
    modes and the answer belongs to neither -- the documented failure of
    soft-argmin disparity regression, and a plausible source of the wells that
    dominate the pooled score.

    Following AcfNet, the target is built as an explicitly unimodal
    distribution ``softmax(-|level - truth| / sigma)`` whose width is *learned*
    per column from a confidence head, rather than the fixed-width smoothing the
    archived alignment loss uses.  Where the evidence is decisive the model can
    narrow the hump; where it is not, it must widen one hump rather than
    quietly splitting into two.  A small penalty on low confidence stops the
    degenerate solution of making every hump maximally wide.
    """

    params = unimodal_loss_params(cfg)
    logits = logits.float()
    axis = axis.to(device=logits.device, dtype=logits.dtype).reshape(1, 1, -1)
    bin_width = float(abs(axis[0, 0, 1] - axis[0, 0, 0])) if axis.shape[-1] > 1 else 1.0

    if confidence is None:
        sigma_bins = logits.new_full(target.shape, params["sigma_bins"])
    else:
        confidence = confidence.float().clamp(0.0, 1.0)
        sigma_bins = params["sigma_min_bins"] + params["sigma_bins"] * (1.0 - confidence)
    sigma = (sigma_bins * bin_width).clamp_min(1e-6).unsqueeze(-1)

    distance = (axis - target.float().unsqueeze(-1)).abs()
    target_probs = torch.softmax(-distance / sigma, dim=-1)

    log_prob = F.log_softmax(logits, dim=-1)
    focal = (1.0 - target_probs).clamp_min(1e-4) ** (-params["focal_alpha"])
    per_bin_loss = -(focal * target_probs * log_prob).sum(dim=-1)
    loss = _weighted_masked_mean(per_bin_loss, target_mask, loss_weight, loss_scale_weight)

    if confidence is not None and params["confidence_weight"] > 0.0:
        confidence_penalty = _weighted_masked_mean(
            1.0 - confidence, target_mask, loss_weight, loss_scale_weight
        )
        loss = loss + params["confidence_weight"] * confidence_penalty
    return loss
...
def iterative_refinement_loss(
    ...
    """Supervise every refinement round, weighting later rounds more.

    RAFT's convention: the loss is a geometric sum over iterations so early
    rounds are pulled toward the answer without dominating the gradient, and the
    final round -- the one actually reported -- carries the most weight.
    """

    gamma = float(getattr(cfg, "iters_loss_gamma", 0.8))
    if not 0.0 < gamma <= 1.0:
        raise ValueError(f"iters_loss_gamma must be in (0, 1], got {gamma}")
    total = None
    normalizer = 0.0
    count = len(iter_preds)
    for index, prediction in enumerate(iter_preds):
        weight = gamma ** (count - 1 - index)
        value = masked_regression_loss_values(
            prediction,
            target,
            target_mask,
            "huber",
        ...
        normalizer += weight
    if total is None:
        raise ValueError("iterative_refinement_loss received no predictions")
...
def _unet_loss_single(
    ...
    if weights["unimodal"] != 0.0:
        if "alignment_logits" not in extra:
            raise ValueError("unimodal loss requires extra['alignment_logits']")
        unimodal_loss = masked_unimodal_cost_loss(
        ...
        weighted = weights["unimodal"] * unimodal_loss
        loss = weighted if loss is None else loss + weighted
        if return_details:
            details["unimodal"] = unimodal_loss

    if weights["iters"] != 0.0:
        if "iter_preds" not in extra:
            raise ValueError("iters loss requires extra['iter_preds']")
        iters_loss = iterative_refinement_loss(
            extra["iter_preds"],
            target,
            target_mask,
            cfg,
        ...
        weighted = weights["iters"] * iters_loss
        loss = weighted if loss is None else loss + weighted
        if return_details:
            details["iters"] = iters_loss
