...
from seq_NN_trf_unet import PretrainedTransformerUNet2d
...
class ConvGRUCell1d(nn.Module):
    """A ConvGRU cell whose hidden state is a Typewell-axis feature map."""

    def __init__(self, input_dim, hidden_dim, kernel_size=5):
        super().__init__()
        kernel_size = int(kernel_size)
        if kernel_size <= 0 or kernel_size % 2 == 0:
            raise ValueError(f"ConvGRU kernel_size must be a positive odd integer, got {kernel_size}")
        ...
        with torch.no_grad():
            self.gates.bias[: self.hidden_dim].fill_(1.0)

    def forward(self, x, hidden):
        if hidden is None:
            hidden = x.new_zeros(x.shape[0], self.hidden_dim, x.shape[-1])
        gate_input = torch.cat([x, hidden], dim=1)
        update, reset = self.gates(gate_input).chunk(2, dim=1)
...
class BiConvGRULayer1d(nn.Module):
    """Bidirectional horizontal-bin scan retaining the complete Typewell axis."""

    def __init__(self, input_dim, hidden_dim, kernel_size=5, act="silu", dropout=0.0, norm="LN"):
        super().__init__()
        hidden_dim = int(hidden_dim)
        if hidden_dim <= 0 or hidden_dim % 2 != 0:
            raise ValueError(f"bidirectional ConvGRU hidden_dim must be a positive even integer, got {hidden_dim}")
        direction_dim = hidden_dim // 2
        self.forward_cell = ConvGRUCell1d(input_dim, direction_dim, kernel_size=kernel_size)
        self.reverse_cell = ConvGRUCell1d(input_dim, direction_dim, kernel_size=kernel_size)
        self.dropout = nn.Dropout(float(dropout))
        self.output_norm = _make_resblock_norm(norm, hidden_dim, ndim=2)
        self.skip = nn.Conv2d(input_dim, hidden_dim, kernel_size=1) if int(input_dim) != hidden_dim else nn.Identity()
        self.output_act = _make_resblock_act(act)

    @staticmethod
    def _scan(sequence, cell, valid_mask=None, reverse=False):
        batch_size, _, horizontal_len, typewell_len = sequence.shape
        hidden = sequence.new_zeros(batch_size, cell.hidden_dim, typewell_len)
        outputs = []
        indices = range(horizontal_len - 1, -1, -1) if reverse else range(horizontal_len)
        for index in indices:
            candidate = cell(sequence[:, :, index, :], hidden)
            if valid_mask is None:
                hidden = candidate
                output = hidden
            else:
                valid = valid_mask[:, index].reshape(batch_size, 1, 1)
                hidden = torch.where(valid, candidate, hidden)
                output = torch.where(valid, hidden, torch.zeros_like(hidden))
    ...
    def forward(self, x, valid_mask=None):
        forward = self._scan(x, self.forward_cell, valid_mask=valid_mask, reverse=False)
        reverse = self._scan(x, self.reverse_cell, valid_mask=valid_mask, reverse=True)
        output = torch.cat([forward, reverse], dim=1)
        residual = self.skip(x)
        return self.output_act(self.output_norm(self.dropout(output)) + residual)


class BiConvGRUEncoder2d(nn.Module):
    """Full-resolution 2D encoder with bidirectional scans over horizontal bins."""
    ...
    def __init__(
        ...
        if norm not in {"BN", "IN", "LN"}:
            raise ValueError(f"unknown ConvGRU norm={norm!r}; expected 'BN', 'IN', or 'LN'")
        self.in_ch = int(in_ch)
        self.hidden_dim = hidden_dim
        self.layers = layers
        self.stem = Conv1dBNAct(
            self.in_ch,
            hidden_dim,
            kernel_size=kernel_size,
            dropout=dropout,
    ...
    def forward(self, x, valid_mask=None):
        batch_size, channels, horizontal_len, typewell_len = x.shape
        if valid_mask is not None and tuple(valid_mask.shape) != (batch_size, horizontal_len):
            raise ValueError(
                f"valid_mask must have shape {(batch_size, horizontal_len)}, got {tuple(valid_mask.shape)}"
            )
        stem = x.permute(0, 2, 1, 3).reshape(batch_size * horizontal_len, channels, typewell_len)
        stem = self.stem(stem)
        x = stem.reshape(batch_size, horizontal_len, self.hidden_dim, typewell_len).permute(0, 2, 1, 3)
...
from seq_NN_arch_blocks import (
    AxialMDStack,
    IterativeLevelRefiner,
    apply_anisotropic_large_kernels,
)
...
class SeqUNet2DModel(nn.Module):
    def __init__(
        ...
        typewell_gr_std=1.0,
        drop_path_rate=0.0,
        arch_unimodal_confidence=False,
        arch_axial_md_stages=(),
        arch_axial_md_depth=1,
        arch_axial_md_heads=4,
        arch_axial_md_mlp_ratio=2.0,
        arch_large_kernel_md=(),
        arch_large_kernel_level=3,
        arch_refine_iters=0,
        arch_refine_hidden=96,
        arch_refine_context=96,
        arch_refine_radius=4,
        arch_refine_dilations=(1, 4, 16),
    ):
        super().__init__()
        ...
        self.deep_sup_stage_decay = float(deep_sup_stage_decay)
        if not math.isfinite(self.deep_sup_stage_decay) or not 0.0 <= self.deep_sup_stage_decay <= 1.0:
            raise ValueError(
                "deep_sup_stage_decay must be finite and in [0, 1], "
                f"got {deep_sup_stage_decay!r}"
            )
        if self.deep_sup and unet_arch != "convnext_small":
            raise ValueError("deep_sup=True is supported only for unet_arch='convnext_small'")
        ...
        if self.deep_sup:
            stage_channels = tuple(self.unet.deep_sup_stage_channels)
            self.deep_sup_projections = nn.ModuleList(
                nn.Identity() if int(stage_channel) == int(unet_emb_dim) else nn.Conv2d(
                    int(stage_channel),
                    int(unet_emb_dim),
                    kernel_size=1,
                    bias=False,
        ...
        # Each is off by default and, when on, starts numerically identical to
        # the archived model: the large-kernel and axial branches are gated by
        # zero-initialised scales, and the refiner's first correction is zero.
        self.arch_large_kernel_md = tuple(int(k) for k in (arch_large_kernel_md or ()))
        if self.arch_large_kernel_md:
            backbone_stages = getattr(self.unet, "backbone_stages", None)
            if backbone_stages is None:
                raise ValueError(
                    "arch_large_kernel_md requires the pretrained ConvNeXt U-Net "
                    "(unet_arch='convnext_small')"
                )
            apply_anisotropic_large_kernels(
                backbone_stages,
                self.arch_large_kernel_md,
                level_kernel=int(arch_large_kernel_level),
            )

        self.arch_axial_md_stages = tuple(int(i) for i in (arch_axial_md_stages or ()))
        if self.arch_axial_md_stages:
            backbone_stages = getattr(self.unet, "backbone_stages", None)
            stage_channels = getattr(self.unet, "stage_channels", None)
            if backbone_stages is None or stage_channels is None:
                raise ValueError(
                    "arch_axial_md_stages requires the pretrained ConvNeXt U-Net "
                    "(unet_arch='convnext_small')"
                )
            for stage_idx in self.arch_axial_md_stages:
                if not 0 <= stage_idx < len(backbone_stages):
                    raise ValueError(
                        f"arch_axial_md_stages index {stage_idx} out of range "
                        f"for {len(backbone_stages)} stages"
                    )
                backbone_stages[stage_idx] = nn.Sequential(
                    backbone_stages[stage_idx],
                    AxialMDStack(
                        int(stage_channels[stage_idx]),
                        depth=int(arch_axial_md_depth),
                        num_heads=int(arch_axial_md_heads),
                        mlp_ratio=float(arch_axial_md_mlp_ratio),
                    ),
                )

        self.arch_unimodal_confidence = bool(arch_unimodal_confidence)
        self.confidence_head = None
        if self.arch_unimodal_confidence:
            # One confidence per MD column, read off the level-pooled features.
            self.confidence_head = nn.Sequential(
                nn.Conv1d(unet_emb_dim * 2, unet_emb_dim, kernel_size=3, padding=1),
                nn.GELU(),
                nn.Conv1d(unet_emb_dim, 1, kernel_size=3, padding=1),
            )
            nn.init.zeros_(self.confidence_head[-1].weight)
            nn.init.zeros_(self.confidence_head[-1].bias)

        self.arch_refine_iters = int(arch_refine_iters)
        self.level_refiner = None
        if self.arch_refine_iters > 0:
            if self.regression_head_mode not in {"logits", "prob+offset"}:
                raise ValueError("arch_refine_iters requires a logits regression head")
            self.level_refiner = IterativeLevelRefiner(
                feature_dim=int(unet_emb_dim),
                hidden_dim=int(arch_refine_hidden),
                context_dim=int(arch_refine_context),
                num_iters=self.arch_refine_iters,
                lookup_radius=int(arch_refine_radius),
                lookup_dilations=tuple(int(d) for d in arch_refine_dilations),
            )
    ...
    def model_modules(self):
        ...
        if len(self.deep_sup_projections) > 0:
            modules.append(self.deep_sup_projections)
        ...
        if confidence_head is not None:
            modules.append(confidence_head)
        level_refiner = getattr(self, "level_refiner", None)
        if level_refiner is not None:
            modules.append(level_refiner)
        return tuple(modules)

    def model_named_parameters(self):
        module_names = [("unet", self.unet), ("alignment_head", self.alignment_head)]
        if len(self.deep_sup_projections) > 0:
            module_names.append(("deep_sup_projections", self.deep_sup_projections))
        if self.regression_logits_head is not None:
        ...
        confidence_head = getattr(self, "confidence_head", None)
        if confidence_head is not None:
            module_names.append(("confidence_head", confidence_head))
        level_refiner = getattr(self, "level_refiner", None)
        if level_refiner is not None:
            module_names.append(("level_refiner", level_refiner))
        seen = set()
    ...
    def _level_axis_scale(self, dtype, device):
        """Affine map from level-bin index to normalised target units."""

        axis = self.typewell_tvt_axis.to(device=device, dtype=dtype)
        if axis.numel() < 2:
            return axis.new_tensor(0.0), axis[0]
        return axis[1] - axis[0], axis[0]
    ...
    def _forward_from_features(self, x, extra_return=False):
        x_pooled = None
        ...
        # Iterative refinement replaces the one-shot expectation as the answer,
        # but keeps it as the starting point, so the refiner can only improve on
        # what the archived head would have said.
        iter_preds = None
        confidence = None
        level_refiner = getattr(self, "level_refiner", None)
        if level_refiner is not None:
            if regression_logits is None:
                raise ValueError("iterative refinement requires regression logits")
            step, origin = self._level_axis_scale(pred.dtype, pred.device)
            init_index = (pred - origin) / step
            init_index = init_index.clamp(0.0, float(regression_logits.shape[-1] - 1))
            iter_preds = level_refiner(
                x,
                regression_logits.float(),
                init_index.float().detach(),
                (step.float(), origin.float()),
            )
            iter_preds = [value.to(pred.dtype) for value in iter_preds]
            pred = iter_preds[-1]

        confidence_head = getattr(self, "confidence_head", None)
        if confidence_head is not None:
            pooled = torch.cat([x.mean(dim=-1), x.amax(dim=-1)], dim=1)
            confidence = torch.sigmoid(confidence_head(pooled).squeeze(1))
        ...
        extra = {
            ...
            "typewell_axis": self.typewell_tvt_axis,
        }
        if iter_preds is not None:
            extra["iter_preds"] = iter_preds
    ...
    def forward(
        ...
        del typewell_aux, z_rel, bin_count, row_mask, z_diff
        unet_input = self._unet_input(unet_static)
        if not self.deep_sup or not extra_return:
            x = self.unet(unet_input)
            return self._forward_from_features(x, extra_return=extra_return)

        x, stage_features = self.unet(unet_input, return_stages=True)
        pred, extra = self._forward_from_features(x, extra_return=True)
        stage_outputs = []
        for projection, stage_feature in zip(self.deep_sup_projections, stage_features):
            stage_feature = projection(stage_feature)
            ...
            stage_pred, stage_extra = self._forward_from_features(
                stage_feature,
                extra_return=True,
            )
            stage_outputs.append({"pred": stage_pred, **stage_extra})
        extra["deep_sup_stages"] = tuple(stage_outputs)
        return pred, extra


class SeqTransformerUNetModel(SeqUNet2DModel):
    """Sequence posterior heads on a timm feature-pyramid/ResBlock U-Net."""
    ...
    def __init__(
        ...
        **shared_kwargs,
        ...
        if pretrained is not None and not isinstance(pretrained, bool):
            raise ValueError(f"pretrained must be boolean or None, got {pretrained!r}")
        forbidden_norm_keys = sorted(
            key for key in shared_kwargs if key.startswith("unet_convnext_")
        )
        if forbidden_norm_keys:
            raise ValueError(
                "trf_unet keeps native backbone normalization and does not accept "
                f"ConvNeXt normalization-replacement settings: {forbidden_norm_keys}"
            )
        super().__init__(
            unet_static_in_dim=unet_static_in_dim,
            unet_emb_dim=int(emb_dim),
            unet_mults=(1,),
            unet_stacks=1,
            unet_arch="unet",
            res_kernel_size=int(res_kernel_size),
            resblock_act=resblock_act,
            resblock_norm=resblock_norm,
            unet_dropout=float(dropout),
            head_hidden_mult=head_hidden_mult,
            updown_mode_2d="non-param",
            regression_head_mode=regression_head_mode,
            share_logits_head=share_logits_head,
            offset_detach_logits_head_in_loss=offset_detach_logits_head_in_loss,
            unet_channel_mask_prob=channel_mask_prob,
            unet_channel_mask_all_prob=channel_mask_all_prob,
            unet_channel_mask_value=channel_mask_value,
            deep_sup=False,
            deep_sup_stage_decay=deep_sup_stage_decay,
            typewell_len=int(typewell_len),
            drop_path_rate=0.0,
            **shared_kwargs,
        )
        del self.unet
        self.unet_arch = "trf_unet"
        self.deep_sup = deep_sup
        self.deep_sup_stage_decay = float(deep_sup_stage_decay)
        self.unet = PretrainedTransformerUNet2d(
            in_ch=int(unet_static_in_dim),
            emb_dim=int(emb_dim),
            input_size=(int(num_bins), int(typewell_len)),
            backbone_name=backbone_name,
            pretrained=bool(pretrained),
            pad_value=float(pad_value),
            kernel_size=int(res_kernel_size),
            resblock_act=resblock_act,
            resblock_norm=resblock_norm,
        ...
        if self.deep_sup:
            self.deep_sup_projections = nn.ModuleList(
                nn.Identity() if int(stage_channel) == int(emb_dim) else nn.Conv2d(
                    int(stage_channel),
                    int(emb_dim),
                    kernel_size=1,
                    bias=False,
                )
                for stage_channel in self.unet.deep_sup_stage_channels
            )
        else:
            self.deep_sup_projections = nn.ModuleList()


class SeqBiConvGRUModel(SeqUNet2DModel):
    """Bidirectional ConvGRU sequence model with the shared posterior head contract."""
    ...
    def __init__(
        ...
        hidden_dim = int(bidir_convgru_hidden_dim)
        super().__init__(
            unet_static_in_dim=unet_static_in_dim,
            unet_emb_dim=hidden_dim,
            unet_mults=(1,),
            unet_stacks=1,
            unet_arch="unet",
            res_kernel_size=int(bidir_convgru_kernel_size),
            resblock_act=bidir_convgru_act,
            resblock_norm=bidir_convgru_norm,
            unet_dropout=float(bidir_convgru_dropout),
            head_hidden_mult=head_hidden_mult,
            regression_head_mode=regression_head_mode,
            share_logits_head=share_logits_head,
            offset_detach_logits_head_in_loss=offset_detach_logits_head_in_loss,
            unet_channel_mask_prob=unet_channel_mask_prob,
            unet_channel_mask_all_prob=unet_channel_mask_all_prob,
            unet_channel_mask_value=unet_channel_mask_value,
            typewell_window=typewell_window,
            typewell_len=typewell_len,
            target_mean=target_mean,
            target_std=target_std,
            tvt_diff_clip=tvt_diff_clip,
            tvt_rel_norm_mean=tvt_rel_norm_mean,
            tvt_rel_norm_std=tvt_rel_norm_std,
            typewell_gr_mean=typewell_gr_mean,
            typewell_gr_std=typewell_gr_std,
        )
        self.unet = BiConvGRUEncoder2d(
            in_ch=int(unet_static_in_dim),
            hidden_dim=hidden_dim,
            layers=int(bidir_convgru_layers),
            kernel_size=int(bidir_convgru_kernel_size),
            dropout=float(bidir_convgru_dropout),
            norm=bidir_convgru_norm,
            act=bidir_convgru_act,
        )
        self.bidir_convgru_hidden_dim = hidden_dim
        self.bidir_convgru_layers = int(bidir_convgru_layers)
        self.bidir_convgru_kernel_size = int(bidir_convgru_kernel_size)
    ...
    def forward(
        ...
        del typewell_aux, z_rel, z_diff
        unet_input = self._unet_input(unet_static)
        valid_mask = None
        if row_mask is not None:
            valid_mask = row_mask > 0
        x = self.unet(unet_input, valid_mask=valid_mask)
        pred, extra = self._forward_from_features(x, extra_return=True)
        if not extra_return:
            return pred
        return pred, extra
