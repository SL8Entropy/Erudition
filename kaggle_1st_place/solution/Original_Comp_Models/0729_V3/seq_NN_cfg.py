...
MODEL_MODES = (UNET_MODE, TWO_STAGE_UNET_MODE, BIDIR_CONVGRU_MODE)
...
class CFG:
    ...
    # Model.
    model_name = "unet" # "unet"/"two_stage_unet"/"bidir_convgru"
    ...
    def set_model_name(self, model_name):
        ...
        elif model_name == TWO_STAGE_UNET_MODE:
            self.model_cfg = self.two_stage_unet_cfg
        else:
            self.model_cfg = self.bidir_convgru_cfg
...
def _validate_seq_train_cfgs(cfgs):
    ...
    for name, cfg in cfgs:
        ...
        if deep_sup and (model_name != UNET_MODE or unet_arch != "convnext_small"):
            raise ValueError(
                f"{name}: deep_sup=True is supported only for model_name='unet' "
...
def make_seq_requested_ablation_cfgs():
    cfgs = [
        ...
        (
            "submit_ver_0729_V2",
            make_static_channel_add_cfg(
                static_add=("pf_prob", "pf_tvt_ffbsi"),
                epochs=200,
                min_epochs=150,
            ),
        ),
        (
            "submit_ver_0729_V3",
            make_static_channel_add_cfg(
                static_add=("pf_particle_density_prob", "pf_tvt_ffbsi"),
                epochs=200,
                min_epochs=150,
            ),
        ),
