...
from seq_NN_models import SeqBiConvGRUModel, SeqTwoStageUNetModel, SeqUNet2DModel
...
def resolve_backbone_frozen_epochs(cfg):
    """Return the active ConvNeXt freeze window, or ``None`` when inapplicable."""
...
def _set_convnext_backbone_requires_grad(model, enabled):
    stages = _convnext_backbone_stages(model)
    if stages is None:
        return False
    for parameter in stages.parameters():
...
def validate_seq_target_support(cfg):
    ...
    if deep_sup and (model_name != UNET_MODE or unet_arch != "convnext_small"):
        raise ValueError(
            "deep_sup=True is supported only for model_name='unet' "
            "with unet_arch='convnext_small'"
        )
...
def run_training(model, train_loader, val_loader, cfg, log, fold_name, finetune_loader=None):
    ...
    def run_phase(
        *,
        ...
        backbone_stages = _convnext_backbone_stages(model)
        if backbone_frozen_epochs is not None and backbone_frozen_epochs > 0:
            if backbone_stages is None:
                raise ValueError(
                    "backbone_frozen_epochs is active but the model has no ConvNeXt backbone"
                )
            backbone_parameter_count = sum(
                parameter.numel() for parameter in backbone_stages.parameters()
            )
            log(
                f"{phase_label}: ConvNeXt backbone parameters frozen for epochs "
        ...
        _set_convnext_backbone_requires_grad(model, True)
        ...
        for epoch in range(1, int(epochs) + 1):
            ...
            if should_freeze_backbone != backbone_is_frozen:
                _set_convnext_backbone_requires_grad(model, not should_freeze_backbone)
