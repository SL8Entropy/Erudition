...
def _build_dsxy_support(
    ...
    if _is_wls_method(method):
        if wls_normalize_dxy:
            ...
            eq_dx = np.full_like(dx, np.nan)
            eq_dy = np.full_like(dy, np.nan)
            eq_ds = np.full_like(ds, np.nan)
            np.divide(dx, step_norm, out=eq_dx, where=denom > 1e-10)
            np.divide(dy, step_norm, out=eq_dy, where=denom > 1e-10)
            np.divide(ds, step_norm, out=eq_ds, where=denom > 1e-10)
        else:
            eq_dx = dx
            eq_dy = dy
            eq_ds = ds
        values = np.column_stack([well.x, well.y, eq_dx, eq_dy, eq_ds, gx, gy]).astype(
            np.float32,
            copy=False,
        )
    elif method == "idw_dS_xy":
        values = np.column_stack([well.x, well.y, gx, gy]).astype(np.float32, copy=False)
    else:
    ...
    if merged.shape[0] == 0:
        y_dim = 5 if _is_wls_method(method) else 2
        return Support(np.empty((0, 2), dtype=np.float32), np.empty((0, y_dim), dtype=np.float32))
    return Support(merged[:, :2], merged[:, 2:])
...
def _build_prior_for_query_well(
    ...
    if raw_idx.size == 0:
        return GeoPriorResult(out, geo_nbr_std, geo_nbr_dS_std, geo_radial_extrap_score)
    ...
    bin_grad, bin_diagnostics = _predict_idw(support.xy, support.y, query_xy, query_dxy, cfg)
