...
class CFG:
    ...
    sim_cfg={
        ...
        'z_shift':{
            ...
            'nan_mode':'same', # 'same'/'none'/'global'
            'TVT_bias_std':0.95,
...
def _validate_seq_train_cfgs(cfgs):
    ...
    valid_z_nan_modes = {"same", "none", "global"}
    ...
    for name, cfg in cfgs:
        ...
        if z_nan_mode not in valid_z_nan_modes:
            raise ValueError(f"{name}: unknown z_shift nan_mode={z_nan_mode!r}")
...
def make_seq_requested_ablation_cfgs():
    downsample_32_kwargs = {
        "downsample": 32,
        "batch_size": 16,
        "val_batch_size": 32,
    }
    downsample_32_accum_kwargs = {
        **downsample_32_kwargs,
        "grad_accum_steps": 2,
    }
    ...
    cfgs = [
        (
            "downsample_32_batch_size_16_val_batch_size_32_grad_accum_steps_2",
            make_unet_cfg(**downsample_32_accum_kwargs),
        ),
        (
            "downsample_32_batch_size_16_val_batch_size_32_grad_accum_steps_2_"
            "z_shift_noise_mode_decomposed_GR",
            make_sim_cfg(
                z_shift={"noise_mode": {"decomposed_GR": 1.0}},
                **downsample_32_accum_kwargs,
            ),
        ),
        (
            "downsample_32_batch_size_16_val_batch_size_32_grad_accum_steps_1_"
            "optimizer_lr_min_lr_half",
            make_optimizer_cfg(
                shared={
                    "lr": float(CFG.optimizer_cfg["lr"]) / 2.0,
                    "min_lr": float(CFG.optimizer_cfg["min_lr"]) / 2.0,
                },
                grad_accum_steps=1,
                **downsample_32_kwargs,
            ),
        ...
        (
            "downsample_32_batch_size_16_val_batch_size_32_grad_accum_steps_2_"
            "unet_convnext_norm_replace_LN_resblock_norm_LN",
            make_unet_cfg(
                model_overrides={
        ...
        (
            "downsample_32_batch_size_16_val_batch_size_32_grad_accum_steps_2_"
            "optimizer_constant_ratio_0p3",
            make_optimizer_cfg(
                shared={"constant_ratio": 0.3},
                **downsample_32_accum_kwargs,
            ),
        ),
    ]
